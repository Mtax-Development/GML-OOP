...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	static construct = function()
	{
		...
		if (argument_count > 0)
		{
			if (is_instanceof(argument[0], TextRenderer))
			{
				...
				ID = _other.ID;
				font = ((is_instanceof(_other.font, Font)) ? new Font(_other.font.ID) : _other.font);
				location = ((is_instanceof(_other.location, Vector2)) ? new Vector2(_other.location)
																	  : _other.location);
			...
			else
			{
				...
				color = (((argument_count > 6) and (argument[6] != undefined)) ? argument[6]
																			   : c_white);
				alpha = (((argument_count > 7) and (argument[7] != undefined)) ? argument[7] : 1);
			}
		}
	...
	static isFunctional = function()
	{
		return ((is_real(alpha)) and (is_instanceof(font, Font)) and (font.isFunctional()) and
				...
				(is_instanceof(align, TextAlign)) and (align.isFunctional()) and ((is_real(scale)) or
				((is_instanceof(scale, Scale)) and (scale.isFunctional()))) and
				((is_instanceof(angle, Angle)) and (angle.isFunctional())));
	}
   ...
   #region <Getters>
	...
	///						renderer as a Scale. If a Scale is specified, its values will be returned
	///						in a new Scale.
	...
	/// @description		Return the number of pixels the text would occupy when rendered with
	///						either its current or specified Scale, or specified Font size.
	...
	/// @description		Return a boundry for the space the text occupies in pixels, offset from
	///						its origin, using either its current or specified Scale or Font size.
   ...
   #region <Setters>
	...
	/// @description		Attempt to fit the text into the specified boundary by adding line breaks
	///						between words, split by the specified separator, if they would exceed
	///						the boundary horizontally if rendered using current or specified
	///						temporarily replaced location and scale. The text will be cut if it would
	///						then exceed vertical boundary.
	static wrapText = function(_boundary, _location = location, _scale = scale, _mark_separator = " ",
							   _mark_cut = "...")
	{
		...
		try
		{
				 ...
				 self.getBoundaryOffset(_scale).x1) > _boundary.x)
			{
				...
				repeat (_word_count)
				{
					//|Add separator before the word unless it is the first one or previous ended
					// with a line break.
					var _word_target =
					(
						((_i > 0) and (!((string_count(_newLine, _word[(_i - 1)]) > 0) and
						(string_last_pos(_newLine, _word[_i - 1]) == (string_length(_word[_i - 1]) -
																	  _newLine_length - 1)))))
   ...
   #region <Execution>
	...
	/// @argument			color? {int:color}
	/// @argument			alpha? {real}
	/// @description		Execute the draw of the text, using data of this constructor or specified
				...
				or (_angle_value != 0))
				{
					...
					else
					{
						...
						draw_text_transformed(_location_x, _location_y, string(ID),
											  _scale_multiplier.x, _scale_multiplier.y, _angle_value);
					}
				}
				else
				{
   ...
   #region <Conversion>
							   ...
							   _elementLength = 30, _mark_cut = "...", _mark_linebreak = ", ")
	{
		...
		var _linebreak_isString = (is_string(_mark_linebreak));
		
		if ((!_multiline) or (_linebreak_isString))
		{
			if (!_linebreak_isString)
			{
				_mark_linebreak = " ";
			};
				...
				else
				{
					...
					if (_char_data != undefined)
					{
						...
						var _vertex_location = [[_location_x_offset, _location_y],
												...
												[_location_x_offset, (_location_y + _char_size_y)],
												[(_location_x_offset + _char_size_x),
												 (_location_y + _char_size_y)]];
						var _vertex_uv = [[_char_uv_x1, _char_uv_y1], [_char_uv_x2, _char_uv_y1],
										  [_char_uv_x1, _char_uv_y2], [_char_uv_x2, _char_uv_y2]];
						_i[2] = 0;
						repeat (_vertex_count)
						{
							...
							array_push(_line_vertexData[_line_index],
									   [_vertex_location[_vertex_index][0],
										_vertex_location[_vertex_index][1], _uv_x, _uv_y, _offset_x]);
				...
				repeat (array_length(_line_vertexData))
				{
					...
					if (_lineData_count > 0)
					{
						...
						repeat (_lineData_count)
						{
							...
							var _origin_transformed_x = (_glyphData_current[0] - _align_offset_x -
														 _origin_absolute_x);
							var _origin_transformed_y = (_glyphData_current[1] - _align_offset_y -
														 _origin_absolute_y);
