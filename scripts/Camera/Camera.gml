...
//  @feather	ignore all
{
  #region [Methods]
	...
	static construct = function()
	{
		...
		if (argument_count > 0)
		{
			if (is_instanceof(argument[0], Camera))
			{
				...
				{
					projectionMatrix = new PerspectiveFieldOfViewProjectionMatrix(_other
																				   .projectionMatrix);
				}
				else
				{
   ...
   #region <Execution>
	...
	///						change how much of the scene is fit into the projection, according to size
	///						affecting its aspect ratio and zoom, as well as the clipping plane cutting
		...
		try
		{
			...
			else
			{
				ErrorReport.report([other, self, "createOrtographicProjectionMatrix()"],
								   ("Attempted to add a projection matrix to an invalid Camera: " +
									"{" + string(ID) + "}"));
			}
		}
		catch (_exception)
	...
	///						projection, according to size stretching either dimension of rendering
	///						result and a clipping plane cutting out graphics outside of it.
	...
	///						the projection, according to field of view Angle affecting how wide the
	///						view is, aspect ratio stretching either dimension of rendering result and
	///						a clipping plane cutting out graphics outside of it.
	static createPerspectiveFieldOfViewProjectionMatrix = function(_fieldOfView, _clippingPlane,
																   _aspectRatio)
	{
		try
		{
			if (self.isFunctional())
			{
				projectionMatrix = new PerspectiveFieldOfViewProjectionMatrix
				(
					_fieldOfView, _aspectRatio, _clippingPlane
				);
		...
		{
			ErrorReport.report([other, self, "createPerspectiveFieldOfViewProjectionMatrix()"],
								_exception);
		}
		...
		if (self.isFunctional())
		{
			if (_full)
			{
				...
				var _string = ("ID: " + string(ID) + _mark_separator +
							   "Projection Matrix: " + string(projectionMatrix) + _mark_separator +
			...
			{
				return ((_multiline) ? string(ID) : (instanceof(self) + "(" + string(ID) + ")"));
			}
		}
		else
		{
  ...
  #region [Elements]
	  ...
	  #region [[Methods]]
	   #region <<Management>>
		...
		static construct = function()
		{
			...
			{
				case 1:
					if (string_copy(instanceof(argument[0]), 1, 27) == "OrtographicProjectionMatrix")
					{
						//|Construction type: Constructor copy.
						var _other = argument[0];
						...
						clippingPlane = ((is_instanceof(_other.clippingPlane, Range))
										 ? new Range(_other.clippingPlane) : _other.clippingPlane);
					}
				break;
				default:
		...
		static isFunctional = function()
		{
			...
			return ((_matrix_valid) and ((is_instanceof(size, Vector2)) and (size.isFunctional())) and
					((is_instanceof(clippingPlane, Range)) and (clippingPlane.isFunctional())));
		}
	   ...
	   #region <<Execution>>
		...
		/// @argument			size? {Vector2}
		/// @argument			clippingPlane? {Range}
		/// @description		Build and assign to this constructor a numeric array representing
		///						projection matrix, using either its configuration or specified
		///						temporarily replaced parts.
		static build = function(_size = size, _clippingPlane = clippingPlane)
		{
			try
			{
				ID = matrix_build_projection_ortho(_size.x, _size.y, _clippingPlane.minimum,
												   _clippingPlane.maximum);
			}
			catch (_exception)
			{
				ErrorReport.report([other, parent, "OrtographicProjectionMatrix", "build()"],
								   _exception);
			}
	   ...
	   #region <<Conversion>>
		...
		/// @description		Create a string representing this constructor.
		///						Overrides the string() conversion.
		///						Content will be represented with the properties of this matrix.
		static toString = function(_multiline = false, _full = false)
		{
			...
			{
				var _mark_separator = ((_multiline) ? "\n" : ", ");
				var _string_matrix = ((_full) ? ("Matrix: " + string(ID) + _mark_separator) : "");
	...
	/// @argument			size {Vector2}
	/// @argument			clippingPlane {Range}
	/// @description		Constructs a matrix supplying the Camera with three-dimensional projection
	///						using field of view based on its size and clipping plane.
	//						
	//						Construction types:
	//						- New element
	//						- Empty: {void|undefined}
	  ...
	  #region [[Methods]]
	   #region <<Management>>
		...
		static construct = function()
		{
			...
			{
				case 1:
					if (string_copy(instanceof(argument[0]), 1, 27) == "PerspectiveProjectionMatrix")
					{
						//|Construction type: Constructor copy.
						var _other = argument[0];
	   ...
	   #region <<Execution>>
		...
		static build = function(_size = size, _clippingPlane = clippingPlane)
		{
			try
			{
				ID = matrix_build_projection_perspective(_size.x, _size.y, _clippingPlane.minimum,
														 _clippingPlane.maximum);
			}
			catch (_exception)
			{
				ErrorReport.report([other, parent, "PerspectiveProjectionMatrix", "build()"],
								   _exception);
			}
	...
	/// @argument			clippingPlane {Range}
	/// @description		Constructs a matrix supplying the Camera with three-dimensional projection
	///						using configurable field of view.
	//						
	//						Construction types:
	//						- New element
	//						- Empty: {void|undefined}
	//						- Constructor copy: other {Camera.PerspectiveFieldOfViewProjectionMatrix}
	  ...
	  #region [[Methods]]
	   ...
	   #region <<Execution>>
								...
								_clippingPlane = clippingPlane)
		{
			try
			{
				ID = matrix_build_projection_perspective_fov(_fieldOfView.value, _aspectRatio,
															 _clippingPlane.minimum,
															 _clippingPlane.maximum);
			}
			catch (_exception)
			{
				ErrorReport.report([other, parent, "PerspectiveFieldOfViewProjectionMatrix",
									"build()"], _exception);
			}
	...
	/// @argument			target {Vector3}
	/// @argument			upVector {Vector3}
	/// @description		Constructs a matrix supplying the Camera with information about source and
	///						target location of the view and its orientation.
	//						
	//						Construction types:
	//						- New element
	//						- Empty: {void|undefined}
	...
	function ViewMatrix() constructor
	{
	  #region [[Methods]]
	   ...
	   #region <<Execution>>
		...
		/// @argument			target? {Vector3}
		/// @argument			upVector? {Vector3}
		/// @description		Build and assign to this constructor a numeric array representing view
		///						matrix, using either its configuration or specified temporarily
		///						replaced parts.
		static build = function(_source = source, _target = target, _upVector = upVector)
		{
			try
			{
				ID = matrix_build_lookat(_source.x, _source.y, _source.z, _target.x, _target.y,
										 _target.z, _upVector.x, _upVector.y, _upVector.z);
			}
			catch (_exception)
			{
