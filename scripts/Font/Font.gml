...
//						- Sprite (glyph map): sprite {Sprite}, glyphs {string}, proportional {bool}
//											  separation {int}, antialiasing? {bool}
...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
		...
		if ((argument_count > 0) and (argument[0] != undefined))
		{
			if (is_instanceof(argument[0], Font))
			{
				...
				switch (_other.type)
				{
					...
					break;
					case "file":
						self.construct(_other.fontName, _other.size, _other.bold, _other.italic,
									   _other.glyphs, _other.antialiasing);
					break;
					case "sprite (UTF-8)":
						sprite = ((is_instanceof(_other.sprite, Sprite))
								  ? new Sprite(_other.sprite) : _other.sprite);
						
						self.construct(sprite, _other.first, _other.proportional, _other.separation,
									   _other.antialiasing);
					break;
					case "sprite (glyph map)":
						...
						self.construct(sprite, _other.glyphs, _other.proportional, _other.separation,
									   _other.antialiasing);
					break;
				}
			...
			else if (is_string(argument[0]))
			{
				...
				italic = argument[3];
				glyphs = argument[4];
				antialiasing = (((argument_count > 5) and (argument[5] != undefined)) ? argument[5]
																					  : false);
				signedDistanceFieldSpread = (((argument_count > 6) and (argument[6] != 0))
											 ? argument[6] : undefined);
	...
	/// @returns			{undefined}
	/// @description		Remove the internal information from the memory if this Font is not an
	///						asset Font or if is and the asset deletion was forced. A destroyed asset
	///						Font will be remain unusable until the application is shut down completely
   ...
   #region <Getters>
	...
	/// @description		Get the UV coordinates for this location of this Font on its texture page.
				...
				switch (type)
				{
					case "asset":
						_string = ("Type: " + string(type) + _mark_separator +
								   ...
								   "Italic: " + string(font_get_italic(ID)) + _mark_separator +
					...
					case "file":
						_string = ("Type: " + string(type) + _mark_separator +
								   ...
								   "Antialiasing: " + string(antialiasing) + _mark_separator +
								   "Signed Distance Field Spread: " +
								   string(signedDistanceFieldSpread));
					break;
					case "sprite (UTF-8)":
						_string = ("Type: " + string(type) + _mark_separator +
								   ...
								   "Proportional: " + string(proportional) + _mark_separator +
