...
//  @feather	ignore all
{
  #region [Methods]
	...
	static construct = function()
	{
		...
		if (argument_count > 0)
		{
			if (is_instanceof(argument[0], ParticleType))
			{
				...
				ID = part_type_create();
				shape = _other.shape;
				sprite = ((is_instanceof(_other.sprite, Sprite)) ? new Sprite(_other.sprite.ID)
																 : _other.sprite);
				sprite_animate = _other.sprite_animate;
				sprite_matchAnimation = _other.sprite_matchAnimation;
				scale = new Scale(_other.scale);
				size = ((is_instanceof(_other.size, Range)) ? new Range(_other.size) : _other.size);
				...
				if (sprite != undefined)
				{
					self.setSprite(sprite, sprite_animate, sprite_matchAnimation, sprite_randomize);
				}
   ...
   #region <Setters>
	...
	/// @description		Set the shape property of this Particle Type, which replaces its Sprite if
	///						it is set. This change affects both new and existing particles.
	...
	/// @description		Set the Sprite properties of particles of this Particle Type, which
	///						replaces its shape is it is set. Specified Sprite can be animated using
	///						its own animation speed, unless it is set to match the life time of this
	///						Particle Type. The animation will play from either the start to finish or
	///						randomized frames if specified. This change affects both new and existing
	///						particles.
	...
	/// @description		Set the Scale of particles of this Particle Type. This change affects both
	...
	/// @description		Set the size properties of this Particle Type to multiply the Scale of the
	///						particles. This change affects only new particles.
	...
	/// @description		Set the movement speed properties of particles of this Particle Type to
	///						change amount of distance it moves through towards its direction each
	///						update of its Particle System. This change affects only new particles.
	...
	/// @argument			increase? {real}
	/// @argument			wiggle? {real}
	/// @description		Set the movemement direction properties of particle of this Particle Type,
	///						applied only while the partices have any speed. This change affects only
	///						new particles.
	static setDirection = function(_direction, _increase = 0, _wiggle = 0)
	{
		try
		{
			...
			part_type_direction(ID, _direction_minimum, _direction_maximum, _increase, _wiggle);
	...
	/// @description		Set the Angle propierties of the particle of this Particle Type, which
	///						rotates their visual representation. Specified Angle can be relative to
	///						the direction that is applied only if the particle has any speed. This
	...
	static setAngle = function(_angle, _increase = 0, _wiggle = 0, _relative = false)
	{
		try
		{
			...
			part_type_orientation(ID, _angle_minimum, _angle_maximum, _increase, _wiggle, _relative);
	...
	/// @description		Set the gravity properties of the particles of this Particle Type to move
	///						particles towards the specified direction, while also applying their own
	///						speed. This change affects both new and existing particles.
	...
	/// @description		Set the life length property of particles of this Particle Type, which is
	///						the amount of steps after which they are destroyed. This change affects
	///						only new particles.
	...
	/// @description		Set the color property of this particles of this Particle Type to either
	///						one static color or a gradual gradient color change over the life time of
	///						each particle. This change affects only new particles.
	...
	/// @description		Set the color property of this Particle Type to a random static value
	///						between two colors. This change affects only new particles.
	...
	/// @description		Set the color property of the particles of this Particle Type to a static
	///						value made from a set of three RGB values, each being either a static
	///						value or randomized from a 0-255 Range. This change affects only new
	///						particles.
	static setColorRGB = function(_red, _green, _blue)
	{
		try
		{
			...
			part_type_color_rgb(ID, _red_minimum, _red_maximum, _green_minimum, _green_maximum,
	...
	/// @description		Set the color property of the particles of this Particle Type to a static
	///						value made from a set of three HSV values, each being either a static
	///						value or randomized from a 0-255 Range. This change affects only new
	///						particles.
	static setColorHSV = function(_hue, _saturation, _value)
	{
		try
		{
			var _hue_minimum, _hue_maximum, _saturation_minimum, _saturation_maximum, _value_minimum,
				_value_maximum;
	...
	/// @description		Set the additive blending property of particles of this Particle Type,
	///						which alters their rendering when their locations overlap. This change
	...
	/// @description		Set the alpha property of particle of this Particle Type to either one
	///						static value or a gradual change over the life time of each particle. This
	///						change affects both new and existing particles.
	...
	/// @argument			number? {int}
	/// @description		Set the step stream properties of particles of this Particle Type, which
	///						will cause them to create the specified number of particles of a different
	///						Particle Type each step. This change affects both new and existing
	///						particles.
		...
		try
		{
			...
			else
			{
				ErrorReport.report([other, self, "setStep()"],
										 ("Attempted to set a property causing a Particle Type to" +
										  "create particles of it own type, causing an infinite " +
										  "loop: " +
										  "{" + string(ID) + "}"));
			}
		}
	...
	/// @argument			type {ParticleType}
	/// @argument			number? {int}
	/// @description		Set the death stream properties of particles of this Particle Type, which
	///						will cause them to create the specified number of particles of a different
	///						Particle Type once when their life time ends. This change affects both new
	///						and existing particles.
   ...
   #region <Execution>
	...
	/// @description		Directly create any number of Particles of this type in the specified
	///						location.
	static create = function(_particleSystem, _location, _number = 1, _color)
	{
		try
		{
			...
			and (_particleSystem.isFunctional()))
			{
				...
				{
					part_particles_create(_particleSystem.ID, _location.x, _location.y, ID, _number);
				}
			...
			else
			{
				ErrorReport.report([other, self, "create()"],
								   ("Attempted to create particles using an invalid Particle Type " +
									"or Particle System:" + "\n" +
									"Self: " + "{" + string(self) + "}" + "\n" +
	...
	/// @argument			number? {int}
	/// @argument			color? {int:color}
	/// @description		Directly create any number of Particles of this type in random locations
	///						within the specified two-dimensional Shape.
	static createShape = function(_particleSystem, _shape, _number = 1, _color)
	{
		try
		{
			...
			and (_particleSystem.isFunctional()))
			{
				...
				var __createParticle;
				
				if (is_real(_color))
				{
					__createParticle = function(_particleSystem, _location_x, _location_y,
												_color)
					{
						part_particles_create_color(_particleSystem.ID, _location_x, _location_y, ID,
													_color, 1);
					}
				}
				else
				{
					__createParticle = function(_particleSystem, _location_x, _location_y)
					{
						part_particles_create(_particleSystem.ID, _location_x, _location_y, ID, 1);
					}
				}
				...
				switch (instanceof(_shape))
				{
					case "Rectangle":
						repeat (_number)
						{
							__createParticle(_particleSystem,
											 irandom_range(_shape.location.x1, _shape.location.x2),
											 irandom_range(_shape.location.y1, _shape.location.y2),
											 _color);
						}
					break;
					case "Triangle":
						repeat (_number)
						{
							...
							var _location_x = (((1 - _sqrt) * _shape.location1.x) + ((_sqrt *
											   (1 - _random)) * _shape.location2.x) +
											   ((_sqrt * _random) * _shape.location3.x));
							var _location_y = (((1 - _sqrt) * _shape.location1.y) + ((_sqrt *
											   (1 - _random)) * _shape.location2.y) +
											   ((_sqrt * _random) * _shape.location3.y));
							
							__createParticle(_particleSystem, _location_x, _location_y,
											 _color);
						}
					break;
					case "Line":
						var _distance = point_distance(_shape.location.x1, _shape.location.y1,
						...
						repeat (_number)
						{
							...
							var _location_x = (_shape.location.x1 + lengthdir_x(_length, _angle) +
											   lengthdir_x(_side_position, _side_angle));
							var _location_y = (_shape.location.y1 + lengthdir_y(_length, _angle) +
											   lengthdir_y(_side_position, _side_angle));
							
							__createParticle(_particleSystem, _location_x, _location_y, _color);
						}
					break;
					case "Circle":
						repeat (_number)
						{
							var _radius = (_shape.radius * sqrt(random(1)));
							var _theta = (random(1) * 2 * pi);
							var _location_x = ceil((_shape.location.x + (_radius * cos(_theta))));
							var _location_y = ceil((_shape.location.y + (_radius * sin(_theta))));
					...
					case "Ellipse":
						var _size_x = ((max(_shape.location.x1, _shape.location.x2) -
										min(_shape.location.x1, _shape.location.x2)) * 0.5);
						var _size_y = ((max(_shape.location.y1, _shape.location.y2) -
										min(_shape.location.y1, _shape.location.y2)) * 0.5);
						repeat (_number)
						{
   ...
   #region <Conversion>
	...
	///						Content will be represented with the properties of this Particle Type.
			...
			if (_full)
			{
				...
				_string = ("ID: " + string(ID) + _mark_separator +
						   ...
						   "Sprite Match Animation: " + string(sprite_matchAnimation)
						   							  + _mark_separator +
						   "Sprite Randomize: " + string(sprite_randomize) + _mark_separator +
						   ...
						   "Direction Increase: " + string(direction_increase) + _mark_separator +
						   "Direction Wiggle: " + string(direction_wiggle) + _mark_separator +
						   ...
						   "Gravity Direction: " + string(gravity_direction) + _mark_separator +
