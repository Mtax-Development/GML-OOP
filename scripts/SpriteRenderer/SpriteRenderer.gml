...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	static construct = function()
	{
		...
		if (argument_count > 0)
		{
			...
			else
			{
				...
				frame = (((argument_count > 2) and (argument[2] != undefined)) ? argument[2] : 0);
				scale = (((argument_count > 3) and (argument[3] != undefined)) ? argument[3]
																			   : new Scale(1, 1));
				angle = (((argument_count > 4) and (argument[4] != undefined)) ? argument[4]
																			   : new Angle(0));
				color = (((argument_count > 5) and (argument[5] != undefined)) ? argument[5]
																			   : c_white);
				alpha = (((argument_count > 6) and (argument[6] != undefined)) ? argument[6] : 1);
	...
	{
		return ((is_instanceof(sprite, Sprite)) and (sprite.isFunctional()) and ((_skip_location) or
				((is_instanceof(location, Vector2)) or  (is_instanceof(location, Vector4))) and
				(location.isFunctional())) and (is_real(frame)) and (is_instanceof(scale, Scale)) and
				(scale.isFunctional()) and (is_instanceof(angle, Angle)) and
				(angle.isFunctional()) and ((is_real(color)) or (is_instanceof(color, Color4))) and
				(is_real(alpha)) and ((part == undefined) or ((is_instanceof(part, Vector4)) and
				(part.isFunctional()))) and ((origin == undefined) or
				((is_instanceof(origin, Vector2)) and (origin.isFunctional()))) and
				((target == undefined) or ((is_real(target)) or ((is_instanceof(target, Surface)) and
				 (target.isFunctional())))));
	}
   ...
   #region <Getters>
	...
	/// @argument			origin? {Vector2}
	/// @returns			{real[+]}
	/// @description		Return an array containing nested arrays with point locations of every
	///						vertex of the render of the data of this constructor or its temporarily
	///						replaced parts.
	static getVertexLocation = function(_sprite = sprite, _location = location, _scale = scale,
										_angle = angle, _part = part, _origin = origin)
	{
		var _result = [];
		...
		try
		{
			...
			{
				//|Basic nineslice support. Only supports "Stretch" Tile Mode and does not support
				// scale-reliant origin point rotation.
	...
	/// @argument			part? {Vector4}
	/// @returns			{any[+]}
	/// @description		Return the texture pointer and UV coordinates for every vertex of the
	///						render of the data of this constructor or its specified temporarily
	///						replaced parts.
	static getUV = function(_sprite = sprite, _frame = frame, _part = part)
	{
		...
		var _part_trimmed_x2 = clamp(((_size_x - _part_x2) - (_size_x - (_size_x - _trim.x2)) -
									  _part_x1), 0, _size_x);
		var _part_trimmed_y2 = clamp(((_size_y - _part_y2) - (_size_y - (_size_y - _trim.y2)) -
									  _part_y1), 0, _size_y);
		...
		if (_nineslice.enabled)
		{
			...
			var _nineslice_outer_topLeft_x = [(_uv_x1 + (_nineslice.left * _texel_x)), _uv_y1];
			...
			var _nineslice_outer_topRight_x = [(_uv_x2 - (_nineslice.right * _texel_x)), _uv_y1];
			var _nineslice_outer_topRight_y = [_uv_x2, (_uv_y1 + (_nineslice.top * _texel_y))];
			var _nineslice_inner_topRight = [_nineslice_outer_topRight_x[0],
											 _nineslice_outer_topRight_y[1]];
			var _nineslice_outer_bottomLeft_x = [(_uv_x1 + (_nineslice.left * _texel_x)), _uv_y2];
			var _nineslice_outer_bottomLeft_y = [_uv_x1, (_uv_y2 - (_nineslice.bottom * _texel_y))];
			var _nineslice_inner_bottomLeft = [_nineslice_outer_bottomLeft_x[0],
											   _nineslice_outer_bottomLeft_y[1]];
			var _nineslice_outer_bottomRight_x = [(_uv_x2 - (_nineslice.right * _texel_x)), _uv_y2];
			var _nineslice_outer_bottomRight_y = [_uv_x2, (_uv_y2 - (_nineslice.bottom * _texel_y))];
			var _nineslice_inner_bottomRight = [_nineslice_outer_bottomRight_x[0],
												_nineslice_outer_bottomRight_y[1]];
			var _nineslice_left_center = [_nineslice_inner_topLeft[0], _nineslice_center[1]];
			var _nineslice_right_center = [_nineslice_inner_topRight[0], _nineslice_center[1]];
			var _nineslice_top_center = [_nineslice_center[0], _nineslice_inner_topRight[1]];
			var _nineslice_bottom_center = [_nineslice_center[0], _nineslice_inner_bottomRight[1]];
			
			return
			[
				_texture,
				[
					_nineslice_outer_topLeft_x, _nineslice_inner_topLeft, _uv_topLeft, _uv_topLeft,
					_nineslice_inner_topLeft, _nineslice_outer_topLeft_y,
					...
					_nineslice_inner_topLeft, _nineslice_inner_bottomLeft, _nineslice_outer_topLeft_y,
					_nineslice_outer_topLeft_y, _nineslice_inner_bottomLeft,
					_nineslice_outer_bottomLeft_y,
					...
					_nineslice_outer_bottomRight_x, _uv_bottomRight, _nineslice_inner_bottomRight,
					_nineslice_inner_bottomRight, _uv_bottomRight, _nineslice_outer_bottomRight_y,
					...
					_nineslice_right_center, _nineslice_inner_bottomRight, _nineslice_center,
					_nineslice_center, _nineslice_inner_bottomRight, _nineslice_bottom_center,
					
					_nineslice_top_center, _nineslice_center, _nineslice_inner_topLeft,
					_nineslice_inner_topLeft, _nineslice_center, _nineslice_left_center,
					
					_nineslice_center, _nineslice_bottom_center, _nineslice_left_center,
					_nineslice_left_center, _nineslice_bottom_center, _nineslice_inner_bottomLeft
				]
			];
		}
		else
	...
	/// @description		Return an array containg rendering data for each vertex rendered by this
	///						constructor, consisting of its primitive type, location, color and alpha
	///						value, based on the data of this constructor or its specified replaced
	///						parts.
	static getPrimitiveRenderData = function(_sprite = sprite, _location = location, _frame = frame,
											 _scale = scale, _angle = angle, _color = color,
											 _part = part, _origin = origin)
	{
		var _vertex_location = self.getVertexLocation(_sprite, _location, _scale, _angle, _part,
													  _origin);
		...
		var _vertex_color = (((sprite_get_nineslice(_sprite.ID).enabled))
							 ...
							 : [_color_topRight, _color_bottomRight, _color_topLeft, _color_topLeft,
								_color_bottomRight, _color_bottomLeft]);
		
		return event.getPrimitiveRenderData.execute(undefined,
													[[pr_trianglelist, _vertex_location, _vertex_uv,
													  _vertex_color]]);
	}
   ...
   #region <Execution>
	...
	static render = function(_sprite = sprite, _location = location, _frame = frame, _scale = scale,
							 _angle = angle, _color = color, _alpha = alpha, _part = part,
   ...
   #region <Conversion>
	...
	/// @returns			{real[+]}
	/// @description		Return an array containing the values of all properties of this Renderer.
	///						If any of properties contain multiple values, they be returned in a nested
	///						array.
	static toArray = function()
	{
		...
		return [_sprite, _location, frame, _scale, _angle, _color, alpha, _part, _origin, _target];
	}
	...
	static toVertexBuffer = function(_sprite = sprite, _location = location, _frame = frame,
									 ...
									 _part = part, _origin = origin, _vertexBuffer)
	{
		...
		try
		{
			...
			_renderData = _vertexBuffer.createPrimitiveRenderData(_primitive[0], undefined, _texture);
