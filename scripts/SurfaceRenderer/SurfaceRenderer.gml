...
//  @feather	ignore all
{
	#region [Methods]
		#region <Management>
			...
			static isFunctional = function()
			{
				return ((is_instanceof(surface, Surface)) and (surface.isFunctional()) and
						((is_instanceof(location, Vector2)) or
						 (is_instanceof(location, Vector4))) and (location.isFunctional()) and
						(is_instanceof(scale, Scale)) and (scale.isFunctional()) and
						(is_instanceof(angle, Angle)) and (angle.isFunctional()) and
						((is_real(color)) or (is_instanceof(color, Color4))) and (is_real(alpha)) and
		...
		#region <Conversion>
			...
			///						of this constructor or specified temporarily replaced parts.
