...
//						Construction types:
//						- New constructor
//						- From RoundRectangle: roundRectangle {RoundRectangle}
//						- Empty: {void}
...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	static construct = function()
	{
		...
		if (argument_count > 0)
		{
			...
			else if (is_instanceof(argument[0], RoundRectangle))
			{
				//|Construction type: From RoundRectangle.
				var _roundRectangle = argument[0];
				...
				fill_alpha = _roundRectangle.fill_alpha;
				outline_size = _roundRectangle.outline_size;
				outline_color = ((is_instanceof(_roundRectangle.outline_color, Color4))
								 ? new Color4(_roundRectangle.outline_color)
								 : _roundRectangle.outline_color);
				outline_alpha = _roundRectangle.outline_alpha;
				
				if (is_struct(_roundRectangle.event))
				{
					event.beforeRender.setAll(_roundRectangle.event.beforeRender);
					event.afterRender.setAll(_roundRectangle.event.afterRender);
				}
				else
				{
					event = _roundRectangle.event;
				}
			}
			else
			{
				...
				fill_alpha = (((argument_count > 2) and (argument[2] != undefined)) ? argument[2]
																					: 1);
				outline_size = (((argument_count > 3) and (argument[3] != undefined)) ? argument[3]
																					  : 0);
   ...
   #region <Getters>
	...
	{
		return ((is_instanceof(_other, Rectangle)) and (fill_alpha == _other.fill_alpha) and
	...
	///						instances of the specified object or a single specified instance, either
	///						inside of this Shape or extended by its outline. An ID of any matching
	///						instance or {noone} will be returned, unless specified to return them in a
	///						List, in which case any matching IDs will be added at the end of it. These
	///						additions can be specified to be ordered by distance from the center of
	///						this Shape. An instance can be specified to exclude it from the result.
								...
								_listOrdered = false, _includeOutline = false)
	{
		...
		try
		{
				...
				if ((is_real(_excludedInstance)) and (instance_exists(_excludedInstance)))
				{
					with (_excludedInstance)
					{
						collision_rectangle_list(_location.x1, _location.y1, _location.x2,
												 _location.y2, _object, _precise, true, _list.ID,
												 _listOrdered);
					}
				}
				else
			...
			else
			{
				...
				{
					return collision_rectangle(_location.x1, _location.y1, _location.x2, _location.y2,
	...
	static containsPoint = function(_point, _includeOutline = false)
	{
		try
		{
			...
			return point_in_rectangle(_point.x, _point.y, _location.x1, _location.y1, _location.x2,
									  _location.y2);
		}
		catch (_exception)
		{
	...
	static cursorOver = function(_device, _GUI = false, _includeOutline = false)
	{
		try
		{
			...
			return ((_cursor_x == clamp(_cursor_x, _location.x1, _location.x2)) and
					(_cursor_y == clamp(_cursor_y, _location.y1, _location.y2)));
		}
		catch (_exception)
		{
	...
	///						represented at following rray positions, nested in the primary array:
	...
	/// @argument			outline_size? {int}
	/// @returns			{Vector4}
	/// @description		Return the location of outer edge of the outline, using data of this
   ...
   #region <Execution>
	...
	///						constructor or specified temporarily replaced parts.
	static render = function(_location = location, _fill_color = fill_color, _fill_alpha = fill_alpha,
