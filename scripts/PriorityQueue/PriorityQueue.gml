...
///						model, which orders values based on their assigned priority.
//						
//						Construction types:
//						- New constructor
...
//  @feather	ignore all
{
  #region [Methods]
   ...
   #region <Getters>
	...
	///						Returns {undefined} if this Priority Queue does not exists or is empty.
	...
	///						Returns {undefined} if this Priority Queue or the value does not exist.
   ...
   #region <Execution>
	...
	///						It can be treated as read-only for this operation to perform it solely on
	///						its copy, without modifying the original to read the values. They will be
	///						iterated through starting from ones with the highest priority, unless
	///						ascending order is specified.
	...
	/// @description		Remove one or more values with the highest priority in this Priority Queue
	///						and return it. If multiple values were removed, they will be returned in
	///						an array. If no values were removed, {undefined} will be returned.
	...
	/// @argument			count? {int}
	/// @returns			{any|any[]|undefined}
	/// @description		Remove one or more values with the lowest priority in this Priority Queue
	///						and return it. If multiple values were removed, they will be returned in
	///						an array. If no values were removed, {undefined} will be returned.
   ...
   #region <Conversion>
							   ...
							   _mark_sizeSeparator = " - ")
	{
		if (self.isFunctional())
		{
			...
			repeat (min(_size, _elementNumber))
			{
				...
				and ((string_length(_newElement)) > (_string_lengthLimit_cut_multiline)))
				{
					_newElement = (string_copy(_newElement, 1, _string_lengthLimit_cut_multiline) +
								   _mark_cut);
				}
	...
	/// @returns			{any[]}
	/// @description		Create an array with all priorities and values of this Priority Queue.
	...
	/// @argument			array {any[]}
	/// @description		Add priority and value pairs from the specified array to this Priority
	///						Queue. The first dimension of the array must contain priorities and the
	///						second their values. Values that are not provided for a priority will be
	///						set to {undefined}.
