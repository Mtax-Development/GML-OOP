...
///						saved in a property with the same property name in the uniform struct of this
///						constructor. They are contained in the Uniform constructors, used to further
///						update their values. Values set while a Shader is not active will be ignored
///						by its code.
//						
//						Construction types:
//						- New constructor
...
//  @feather	ignore all
{
  #region [Methods]
   ...
   #region <Setters>
	...
	///						floating-point numbers. If only one value argument is passed, it can be an
	///						array, Vector2 or a Vector3. Otherwise all value arguments have to be
	///						numbers.
				...
				else
				{
					...
					else
					{
						ErrorReport.report([other, self, "setUniformFloat()"],
										   ("Attempted to set an uniform using an unrecognized " +
											"data type:" + "\n" +
											"Shader: " + "{" + string(name) + "}" + "\n" +
	...
	/// @argument			value3? {int}
	/// @argument			value4? {int}
	/// @description		Use the specified uniform to pass to this Shader one or more integer
	///						numbers. If only one value argument is passed, it can be an array, Vector2
	...
	/// @argument			uniform {string}
	/// @argument			value? {real[]}
	/// @description		Use the specified uniform to pass to this Shader either the specified
	///						matrix values or the ones of currently set transform matrix.
	...
	/// @description		Use the specified sampler uniform to pass to this Shader the specified
	...
	static setUniformTexture = function(_uniform, _texture)
	{
		try
		{
			...
			if (_handle >= 0)
			{
				...
				self.createUniformData(_uniform, _handle, "sampler", [_texture], texture_set_stage);
			}
		}
		catch (_exception)
		{
	...
	/// @description		Pass to this Shader values contained in uniform struct through uniforms
	///						with the same name as its properties.
   ...
   #region <Execution>
	...
	/// @description		Create a Uniform Handler containing specified data of a uniform of this
	///						Shader, through which its values can be updated.
	static createUniformData = function(_uniform, _handle, _type, _value, _updateFunction)
	{
		try
		{
			variable_struct_set(uniform, _uniform, new Uniform(_uniform, _handle, _type, _value,
															   _updateFunction));
		}
		catch (_exception)
		{
  ...
  #region [Elements]
	...
	/// @argument			updateFunction? {function:shader_set_uniform_*_array|
	///										 function:texture_set_stage}
	/// @description		Constructs a Handler with properties of a single Uniform used to update
	///						values within its parent Shader.
	//						
	//						Construction types:
	//						- New element
	function Uniform() constructor
	{
	  #region [[Methods]]
	   ...
	   #region <<Execution>>
		...
		/// @argument			value? {int[]|real[]|pointer:texture[]}
		/// @description		Set the uniform value within the Shader to the specified or current
		///						value of this constructor if that Shader is currently active. The
		///						value must be an array in the format of executable arguments for a
		///						function call.
