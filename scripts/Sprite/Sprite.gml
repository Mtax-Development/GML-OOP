...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
			...
			else
			{
				if ((is_string(argument[0])) and (file_exists(argument[0])))
				{
					...
					var _removeBackground = (((argument_count > 2) and
											  (argument[2] != undefined)) ? argument[2] : false);
					var _smoothRemovedBackground = (((argument_count > 3) and
													(argument[3] != undefined)) ? argument[3]
																				: false);
					...
					ID = sprite_add(_path, _frameCount, _removeBackground, _smoothRemovedBackground,
									_origin_x, _origin_y);
					name = sprite_get_name(ID);
					size = new Scale(sprite_get_width(ID), sprite_get_height(ID));
					frameCount = sprite_get_number(ID);
				...
				or (is_instanceof(argument[1], Vector4))))
				{
					...
					var _removeBackground = (((argument_count > 3) and (argument[3] != undefined))
											 ? argument[3] : false);
					var _smoothRemovedBackground = (((argument_count > 4) and
													(argument[4] != undefined)) ? argument[4]
																				: false);
	...
	static replace = function(_other)
	{
		try
		{
			...
			{
				boundary = new Vector4(_boundary_x1, _boundary_y1, _boundary_x2, _boundary_y2);
			}
	...
	///						These frames will be resized to the size of this Sprite.
   ...
   #region <Getters>
	...
	/// @description		Return pointers to the texture page of specified frames of this Sprite.
	...
	/// @description		Return the UV coordinates for the location of the specified frames of this
	///						Sprite on its texture page.
	///						They will be returned as an Vector4 if the full information is not
	///						specified. Otherwise, an array with 8 elements will be returned to include
	///						information about fully transparent pixels excluded from its texture page
	///						during compiling. These entries will be nested if information for multiple
	///						frames was specified. The data of full information is represented at
	///						following array positions:
			...
			if (is_real(_frame))
			{
				if (_frame == all)
				{
					...
					repeat (_count)
					{
						...
						_result[_i] = ((_full) ? _uv : new Vector4(_uv[0], _uv[1], _uv[2], _uv[3]));
	...
	static getTexel = function()
	{
		try
		{
			...
			return new Vector2(texture_get_texel_width(_texture), texture_get_texel_height(_texture));
		}
		catch (_exception)
		{
	...
	/// @description		Return the number of fully transparent pixels removed from each side of
	///						the specified frame during compilation, which occurs if this Sprite is in
	///						a texture group with automatic cropping enabled. The result will be
	///						returned in a Vector4, unless a Scale was specified, in which case an
	///						offset from center of this Sprite will be returned in a Vector2,
	///						representing offset of pixels remaining after cropping, multiplied by that
	///						specified Scale.
   ...
   #region <Setters>
	...
	/// @description		Set a Nine Slice struct that is bound to another or no other Sprite to
	///						this Sprite.
	...
	/// @description		Set the origin point, which is an offset from the top-left pixel of this
	///						Sprite added to its render location and the center of rotation.
	...
	///						- Alpha tolerance has to be specified only if the mode is not Full Image
	///						  and will be used to ignore pixels which have the alpha value that equal
	///						  or exceed it.
	///						A separate mask can be created for each frame of this Sprite, which can be
	///						used if their shape or alpha values are to affect the collision mask.
									   ...
									   _alphaTolerance, _separateMasks = false)
	{
		try
		{
			...
			sprite_collision_mask(ID, _separateMasks, _boundaryMode, _boundary_x1, _boundary_y1,
								  _boundary_x2, _boundary_y2, _boundaryType, _alphaTolerance);
   ...
   #region <Execution>
	...
	/// @description		Draw this Sprite or a part of it to the currently active or specified
	///						Surface.
	static render = function(_location, _frame = 0, _scale, _angle, _color = c_white, _alpha = 1,
							 _part, _origin, _target)
	{
		...
		try
		{
			if (self.isFunctional())
			{
				...
				{
					var _target_value = ((is_instanceof(_target, Surface)) ? _target.ID : _target);
				...
				draw_sprite_general(ID, _frame, _part_x1, _part_y1, _part_x2, _part_y2, _location_x,
	...
	/// @argument			alpha? {real}
	/// @description		Draw this Sprite repeatedly tiled through the entire view, the currently
	///						active created Surface or if neither are used, the current Room.
		...
		try
		{
			if (self.isFunctional())
			{
				...
				if (argument_count > 2)
				{
					...
					draw_sprite_tiled_ext(ID, _frame, _offset_x, _offset_y, _scale_x, _scale_y,
	...
	/// @argument			frame? {int}
	/// @argument			alpha? {real}
	/// @description		Draw this Sprite to the currently active Surface with perspective altered
	///						by its boundaries.
	static renderPerspective = function(_location1, _location2, _location3, _location4, _frame = 0,
										_alpha = 1)
	{
		try
		{
			...
			{
				event.beforeRender.execute();
				draw_sprite_pos(ID, _frame, _location1.x, _location1.y, _location2.x, _location2.y,
								_location3.x, _location3.y, _location4.x, _location4.y, _alpha);
				event.afterRender.execute();
			}
			else
			{
   ...
   #region <Conversion>
	...
	static toString = function(_multiline = false, _full = false)
	{
		if (self.isFunctional())
		{
			...
			if (_full)
			{
				var _string_size = string(new Vector2(sprite_get_width(ID), sprite_get_height(ID)));
				...
				_string = ("Name: " + _string_name + _mark_separator +
						   ...
						   "Boundary: " + _string_boundary_mode + _string_boundary + _mark_separator +
	...
	/// @argument			frame? {int}
	/// @description		Save this Sprite to the specified PNG file, either only its one specified
	///						frame or all of them in one file in a horizontal strip.
	///						Applicable only if this Sprite was created during the runtime.
