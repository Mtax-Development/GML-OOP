...
///						axis to fit between two points of its specified location. It is a series of
///						triangles placed circularly from center, the amount of which equals the
///						specified precision.
//						
//						Construction types:
//						- New constructor
  ...
  #region [Methods]
								...
								_listOrdered = false, _includeOutline = false)
	{
		...
		try
		{
			...
			else
			{
				...
				{
					return collision_ellipse(_location.x1, _location.y1, _location.x2, _location.y2,
	...
	static cursorHold = function(_button, _device, _GUI = false, _includeOutline = false)
	{
		try
		{
			return (((_device == undefined) ? mouse_check_button(_button)
											: device_mouse_check_button(_device, _button)) and
					(self.cursorOver(_device, _GUI, _includeOutline)));
		}
		catch (_exception)
		{
   ...
   #region <Conversion>
					...
					switch (_color[_i])
					{
						...
						default:
							...
							else
							{
								_string_color[_i] =
								("(" +
								 "Red: " + string(color_get_red(_color[_i])) + _mark_separator +
								 "Green: " + string(color_get_green(_color[_i])) + _mark_separator +
	...
	static toArray = function()
	{
		...
		var _fill_color = ((is_instanceof(fill_color, Color2)) ? fill_color.toArray() : fill_color);
		
		return [_location, _fill_color, fill_alpha, outline_size, outline_color, outline_alpha,
				precision];
	}
	...
	static toVertexBuffer = function(_location = location, _fill_color = fill_color,
									 ...
									 _precision = precision, _outline = all, _vertexBuffer)
	{
