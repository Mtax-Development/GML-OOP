...
//  @feather	ignore all
{
  #region [Static Properties]
   #region [[Configurable Properties - General]]
	...
	/// @type				{int}?
	/// @description		Maximum number of times the reporting function will be called, compared to
	///						the number of saved report data.
	static reportFunctionLimit = undefined;
	
	/// @type				{int}?
	/// @description		Number of collected error entries after which error data will not be
	///						automatically saved or reported, preventing memory leaks.
	static errorDataLimit = 250;
	
	/// @type				{bool}?
	/// @description		Whether unique report function calls are enforced by first checking if a
  ...
  #region [Methods]
   ...
   #region <Execution>
	...
	/// @argument			error_location? {handle:instance|string|struct|[]}
	/// @argument			error_detail? {string|struct:exception}
	/// @returns			{ErrorReport.ReportData}
	/// @description		Create an error report, log its data with the function assigned to the
	///						appropriate static constructor variable and return that data.
   ...
   #region <Conversion>
	...
	///						Content will be represented with error and configuration data.
	static toString = function(_multiline = false, _full = false)
	{
		if (_full)
		{
			...
			var _text_errorData_length = ((is_array(errorData))
										  ? string(array_length(errorData)) : string(undefined));
			...
			var _string = ("Report Function: " + _text_reportFunction + _mark_separator +
						   "Error Data Saved: " + _text_errorData_length + _text_errorDataLimit +
												  _mark_separator +
						   "Report Function Limit: " + string(reportFunctionLimit) + _mark_separator +
		...
		else
		{
			if (is_array(errorData))
			{
				...
				var _string_errorData_length = ((_errorData_length > 0) ?
												("Reports: " + string(_errorData_length)) : "Empty");
  ...
  #region [Elements]
	...
	/// @argument			location {handle:instance|string|struct|[]}
	...
	function ReportData() constructor
	{
	  #region [[Methods]]
	   #region <<Management>>
		...
		static isFunctional = function()
		{
			return ((location != undefined) and ((is_string(detail)) or
					...
					(is_string(detail[$ "longMessage"])) and (is_string(detail[$ "script"])) and
					(is_array(detail[$ "stacktrace"])) and (array_length(detail.stacktrace) > 0))) and
	   ...
	   #region <<Getters>>
		...
		/// @returns			{bool}
		/// @description		Check if description and callstack of this error are the same as of
		///						the specified one.
		...
		/// @returns			{string}
		/// @description		Create a string representing readable location in code where the error
		///						occurred.
		static formatLocation = function()
		{
			...
			repeat (_error_location_count)
			{
				...
				{
					//|Instance or room {self} reference. Will exist as a pseudo-struct even if
					// destroyed.
		...
		/// @returns			{string}
		/// @description		Return the string describing the detail of the error or parsed error
		///						message if it is an exception struct.
		static formatDetail = function()
		{
			var _text_detail = undefined;
			
			if ((is_struct(detail)) and (is_string(variable_struct_get(detail, "message"))))
			{
		...
		/// @returns			{string}
		/// @description		Return the string representing entire or part of the error callstack,
		///						containing each call in a separate line.
		...
		/// @returns			{string}
		/// @description		Return the string representing the time at which the error occurred,
	   ...
	   #region <<Conversion>>
		...
		static toString = function(_multiline = false, _detailLength = 30, _callstackLength = 10,
								   ...
								   _mark_timeSeparator = ": ")
		{
			...
			if (self.isFunctional())
			{
				...
				var _text_time = ((time != undefined) ? self.formatTime() : "Unknown time");
				...
				if (_detailLength != all)
				{
					...
					{
						_text_detail = (string_copy(_text_detail, 1, _detailLength) + _mark_cut);
					}
				}
				...
				if (_multiline)
				{
					...
					return  ("Location: " + _text_location + _mark_linebreak +
							 ...
							 "Time: " + ((time != undefined) ? self.formatTime() : "Unknown time")
									  + _mark_linebreak +
							 "Callstack: " + self.formatCallstack(_callstackLength));
				}
				else
				{
					return (_constructorName + "(" + string_replace_all(_text_location, "\n", " ") +
							_mark_separator + ((time != undefined)
											   ? (self.formatTime() + _mark_timeSeparator) : "") +
							_text_detail + ")");
				}
			}
			else
