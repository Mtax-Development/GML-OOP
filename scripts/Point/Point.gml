...
//  @feather	ignore all
{
  #region [Methods]
   ...
   #region <Getters>
	...
	///						instances of the specified object or a single specified instance at the
	///						location of this Shape. An ID of any matching instance or {noone} will be
	///						returned, unless specified to return them in a List, in which case any
	///						matching IDs will be added at the end of it. These additions can be
	///						specified to be ordered by distance from the center of this Shape. An
		...
		try
		{
			...
			if (_list)
			{
				...
				if ((is_real(_excludedInstance)) and (instance_exists(_excludedInstance)))
				{
					...
					{
						collision_point_list(_location.x, _location.y, _object, _precise, true,
											 _list.ID, _listOrdered);
					}
				}
				else
				{
					collision_point_list(_location.x, _location.y, _object, _precise, false, _list.ID,
										 _listOrdered);
				}
			...
			else
			{
				if ((is_real(_excludedInstance)) and (instance_exists(_excludedInstance)))
				{
					with (_excludedInstance)
					{
						return collision_point(_location.x, _location.y, _object, _precise, true);
					}
				}
				else
				{
					return collision_point(_location.x, _location.y, _object, _precise, false);
				}
			}
		}
		catch (_exception)
	...
	/// @description		Check if the system cursor location is over this Shape. A target device
	///						can be specified for use with multiple cursor input sources. In that case,
	///						its position can also be specified to be calculated according to current
	///						GUI layer size.
	...
	//  @see				display_set_gui_size()
	/// @description		Check if the system cursor location is over this Shape while its specified
	///						input was pressed this frame. A target device can be specified for use
	///						with multiple cursor input sources. In that case, its position can also be
	///						specified to be calculated according to current GUI layer size.
	...
	//  @see				display_set_gui_size()
	/// @description		Check if the system cursor location is over this Shape while its specified
	///						input was released this frame. A target device can be specified for use
	///						with multiple cursor input sources. In that case, its position can also be
	///						specified to be calculated according to current GUI layer size.
	...
	/// @description		Return an array containg rendering data for vertex resulting in this
	///						Shape, consisting of its primitive type, location, color and alpha value,
	///						based on the data of this constructor or its specified replaced parts.
	///						Each will be represented at following array positions:
	...
	static getPrimitiveRenderData = function(_location = location, _color = color, _alpha = alpha)
	{
		try
		{
			return event.getPrimitiveRenderData
					.execute(undefined, [[pr_pointlist, [[_location.x, _location.y], _color,
										  _alpha]]]);
		}
		catch (_exception)
		{
   ...
   #region <Execution>
	...
	/// @argument			color? {int:color}
	/// @argument			alpha? {real}
	/// @description		Execute the draw of this Shape as a sprite, using data of this constructor
