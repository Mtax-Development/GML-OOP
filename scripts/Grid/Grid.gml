...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	/// @argument			other {Grid}
	/// @description		Replace all data of this Grid with data from another one after resizing
	///						this Grid to match its size.
   ...
   #region <Getters>
	...
	/// @argument			value... {any}
	/// @returns			{bool}
	/// @description		Check if this Data Structure contains at least one of the specified values
	...
	static containsRegion = function(_location)
	{
		try
		{
			...
			{
				if (ds_grid_value_exists(ID, _location.x1, _location.y1, _location.x2, _location.y2,
										 argument[_i]))
				{
					return true;
				}
	...
	static containsDisk = function(_location, _radius)
	{
		try
		{
			...
			{
				if (ds_grid_value_disk_exists(ID, _location.x, _location.y, _radius, argument[_i]))
				{
					return true;
				}
	...
	/// @description		Return either the value of a cell at the specified position in this Grid
	///						or {undefined} if this Grid does not exist or that position is outside it.
	...
	/// @returns			{any[]}
	/// @description		Return all values on the specified x position in an array or an empty
	///						array if that position is outside of this Grid.
	...
	/// @argument			position {int}
	/// @returns			{any[]}
	/// @description		Return all values on the specified y position in an array or an empty
	///						array if that position is outside of this Grid.
	...
	/// @description		Return the lowest numerical value found in cells within the specified
	///						region of this Grid. If the region contains values other than numerical,
	///						it might be found instead and will be returned as {undefined}.
	...
	/// @argument			radius {int}
	/// @returns			{real} | On error: {undefined}
	/// @description		Return the lowest numerical value found in cells within the specified disk
	///						of this Grid. If the disk contains values other than numerical, it might
	///						be found instead and will be returned as {undefined}.
	...
	/// @argument			location? {Vector4}
	/// @returns			{real} | On error: {undefined}
	/// @description		Return the highest numerical value found in cells within the specified
	///						region of this Grid. If the region contains values other than numerical,
	///						it might be found instead and will be returned as {undefined}.
	...
	/// @returns			{real} | On error: {undefined}
	/// @description		Return the highest numerical value found in cells within the specified
	///						disk of this Grid. If the disk contains values other than numerical, it
	///						might be found instead and will be returned as {undefined}.
	...
	/// @returns			{real} | On error: {undefined}
	/// @description		Return the mean number of numerical values found in cells within the
	///						specified disk of this Grid. If the disk contains no numerical values, 0
	///						will be returned.
	...
	///						the specified region of this Grid. Strings containing only numbers will
	///						also be included. Other values will be ignored. If this Grid does not
	///						exist or the region contains no values that can be summed, {undefined}
	///						will be returned.
	...
	/// @argument			radius {int}
	/// @returns			{real|undefined}
	/// @description		Return the sum of numerical values found in cells within the specified
	///						disk of this Grid. Strings containing only numbers will also be included.
	///						Other  values will be ignored. If this Grid does not exist or the disk
	///						contains no values that can be summed, {undefined} will be returned.
	...
	/// @description		Get the location of a cell containing the specified value in the specified
	///						region of this Grid. Returns {undefined} if this Grid or the specified
	...
	/// @returns			{Vector2|undefined}
	/// @description		Get the location of a cell containing the specified value in the specified
	///						disk of this Grid. Returns {undefined} if this Grid or the specified value
	///						does not exist.
	static getValueLocationDisk = function(_value, _location, _radius)
	{
		try
		{
			var _location_x = ds_grid_value_disk_x(ID, _location.x, _location.y, _radius, _value);
			var _location_y = ds_grid_value_disk_y(ID, _location.x, _location.y, _radius, _value);
   ...
   #region <Execution>
	...
	/// @argument			value... {any}
	/// @argument			location... {Vector2}
	/// @description		Replace any number of values in the specified cells in this Grid with the
	///						specified values.
	static set = function()
	{
		var _i = 0;
	...
	/// @argument			value {any}
	/// @argument			location? {Vector4}
	/// @description		Replace the values of cells of the specified region in this Grid with the
	///						specified value.
	static setRegion = function(_value, _location)
	{
		try
		{
			...
			{
				ds_grid_set_region(ID, _location.x1, _location.y1, _location.x2, _location.y2,
								   _value);
			}
		}
		catch (_exception)
	...
	/// @argument			location {Vector2}
	/// @argument			radius {int}
	/// @description		Replace the values of cells of the specified disk in this Grid with the
	///						specified value.
	...
	/// @argument			other? {Grid}
	/// @description		Copy values of cells from the specified region in this or specified other
	///						Grid and replace the values of cells in the region of the same size in
	///						this Grid, starting from the specified target location.
	static setRegionCopied = function(_target, _source, _other = self)
	{
		try
		{
			ds_grid_set_grid_region(ID, _other.ID, _source.x1, _source.y1, _source.x2, _source.y2,
									_target.x, _target.y);
		}
		catch (_exception)
		{
	...
	/// @argument			location... {Vector2}
	/// @description		Add the specified values to any number of values in the specified cells of
	///						this Grid. The values will be replaced if they are not the same type as
	///						the one already existing in the cell.
	...
	/// @argument			value {real|string}
	/// @argument			location? {Vector4}
	/// @description		Add the specified value to values in cells in the specified region of this
	///						Grid. The values will be replaced if they are not the same type as the one
	///						already existing in the cell.
	...
	/// @argument			location {Vector2}
	/// @argument			radius {int}
	/// @description		Add the specified value to values in cells in the specified disk of this
	///						Grid. The values will be replaced if they are not the same type as the one
	///						already existing in the cell.
	...
	/// @argument			source {Vector4}
	/// @argument			other? {Grid}
	/// @description		Copy values of cells from the specified region of this or specified other
	///						Grid and add them to values of cells in the region of the same size in
	///						this Grid, starting from the specified target location. The value will be
	///						replaced if it is not the same type as the one already existing in the
	///						cell.
	static addRegionCopied = function(_target, _source, _other = self)
	{
		try
		{
			ds_grid_add_grid_region(ID, _other.ID, _source.x1, _source.y1, _source.x2, _source.y2,
									_target.x, _target.y);
		}
		catch (_exception)
		{
	...
	/// @argument			value... {real}
	/// @argument			location... {Vector2}
	/// @description		Multiply by the specified values any number of numerical values in the
	///						specified cells of this Grid.
	...
	/// @argument			value {real}
	/// @argument			location? {Vector4}
	/// @description		Multiply by the specified value the numerical values in cells in the
	...
	static multiplyRegion = function(_value, _location)
	{
		try
		{
			if (_location == undefined)
			{
				ds_grid_multiply_region(ID, 0, 0, ds_grid_width(ID), ds_grid_height(ID), _value);
			}
			else
			{
				ds_grid_multiply_region(ID, _location.x1, _location.y1, _location.x2, _location.y2,
										_value);
			}
		}
		catch (_exception)
	...
	/// @argument			other? {Grid}
	/// @description		Copy values of cells from the specified region of this or specified other
	///						Grid and multiply by them the number values of cells in the region of the
	///						same size in this Grid, starting from  the specified target location.
	...
	///						vertical cell column and then applying its sort order to other columns.
   ...
   #region <Conversion>
							   ...
							   _mark_elementEnd = "]", _mark_sizeSeparator = " - ")
	{
		if (self.isFunctional())
		{
			...
			if (!is_real(_elementLength))
			{
				_elementLength = ((_multiline) ? 15 : 30);
			}
			...
			repeat (min(_size_y, _elementNumber))
			{
				...
				repeat (_size_x)
				{
					...
					if ((_elementLength != all) and ((string_length(_newElement)) > _elementLength))
					{
				...
				else if (_elementLength != all)
				{
					//|Cut strings and add cut or separation marks if appropriate.
					var _string_length = string_length(_string);
					
					//|If the current element is not the last, add a separator or cut it if it would
					// be too long.
					if (_y < (_size_y - 1))
					{
						if ((_string_length + _mark_separator_length) > _string_lengthLimit_cut)
						{
							_string = (string_copy(_string, 1, _string_lengthLimit) + _mark_cut);
							
							break;
						}
						else if (_y < (_elementNumber - 1))
						{
					...
					{
						//|If the current element is last, cut it if it would be too long, but expand
						// the length check by the length of the cut mark.
						_string = (string_copy(_string, 1, _string_lengthLimit) + _mark_cut);
	...
	/// @description		Replace this Grid with one that has the size and values of the specified
	///						one-dimensional or two-dimensional array. Since arrays can have columns of
	///						varying sizes and represent no value in some Grid cells, a default value
	...
	/// @returns			{string}
	/// @description		Encode this Data Structure into a string, from which it can be recreated.
