...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	/// @description		Set the value operated by this parser to the specified value or the value
	///						of the specified parser by ensuring it is a string.
   ...
   #region <Getters>
	...
	/// @argument			value... {any:string|StringParser}
	/// @returns			{bool}
	/// @description		Check if at least one occurence of any of the specified filter substrings
	...
	static contains = function()
	{
		...
		repeat (argument_count)
		{
			...
			var _part = ((is_string(_value)) ? _value : ((is_instanceof(_value, StringParser))
	...
	/// @argument			value... {any:string|StringParser}
	/// @returns			{bool}
	/// @description		Check if at least one occurrence of every of the specified substrings are
	...
	/// @description		Check if the character at the specified position is the same as the
	///						specified char or is contained in an array of them.
	...
	/// @argument			position {int}
	/// @returns			{bool} | On error: {undefined}
	/// @description		Check if the character at the specified position in the string is an
	///						invisible character without a glyph.
	...
	///						in an array. Parts of the string can be parsed after separating them and
	///						returned as a {StringParser} if specified as such or by the specified
	///						function, which will be provided each string part as its only argument.
	///						The value of this parser will be set to a string part for the duration of
	///						each execution only. If the separator is not present, a single unseparated
	///						string is returned or {self} if the {StringParser} was specified for
	///						parsing.
	static split = function(_separator, _parse_part)
	{
		try
		{
			...
			{
				var _separator_count =
				(
					(_string_length - string_length(string_replace_all(_string, _separator, ""))) /
					_separator_length
				);
				...
				and (_string_length != (_separator_length * _separator_count)))
				{
					...
					_segment = string_copy(_string, _position, (_string_length - _position + 1));
						...
						else
						{
							...
							if (is_real(_function_index))
							{
								...
								repeat (array_length(_result))
								{
									...
									{
										ErrorReport.report([other, self, "split()", "function()"],
														   _exception);
									}
	...
	/// @description		Return a part of the string between two specified substrings, the start or
	///						the end of the string, excluding parts containing the specified ignored
	///						substring. If the result contains more than one of parts, they will be
	///						returned in an array. An empty string will be returned if it contains no
	///						parts. Used start and end of parts can be specified to be added back to
	///						them.
				...
				if (_token_start_position > 0)
				{
					...
					if (_token_end_position > 0)
					{
						...
						if (string_length(_part) > 0)
						{
							...
							if (!_part_ignore)
							{
								array_push(_result, ((_include_tokens) ? (_token_start + _part +
																		  _token_end) : _part));
							}
						}
					}
	...
	/// @argument			position {int}
	/// @returns			{int}
	/// @description		Return an Unicode (UTF-8) value for a character from the string at the
	///						specified position.
	...
	/// @argument			count? {int}
	/// @returns			{string}
	/// @description		Return a part of the string, starting from the character at the specified
	///						position and continuing forward by the specified count of characters or
	///						its end.
	...
	/// @returns			{string}
	/// @description		Return a variation of the string containing only its letters from the
	///						English alphabet.
	...
	/// @returns			{string}
	/// @description		Return a variation of the string containing only its digits.
	...
	/// @returns			{string}
	/// @description		Return a variation of the string containing only its digits and letters
	...
	/// @argument			substring {any:string}
	/// @returns			{int}
	/// @description		Return a number of instances of the specified substring appearing in the
	///						string.
	...
	/// @description		Return the first position of a substring instance in a string, a specific
	///						number of them in an array, or all of them, starting from the start or end
	///						of the string, or specified offset from either. Positions will equal the
	///						position of the first character of the substring, always counting from the
	///						start of the string. If the string does not contain the specified
	///						substring, {undefined} will be returned.
	...
	/// @returns			{int}
	/// @description		Return the number of characters in the string after parsing out escape
	///						characters.
	...
	/// @argument			width? {int}
	/// @returns			{Vector2}
	/// @description		Return the number of pixels the string would occupy by applying either the
	///						specified or currently set Font, at specified Scale or size of that Font.
	///						Limitations for width before forced line-break and separation between
	///						lines of text can be included by specifying them.
	static getPixelSize = function(_font, _scale, _separation, _width)
	{
		...
		try
		{
			...
			{
				var _font_point_size_multiplier = (round(_scale) / font_get_size(_font_current));
   ...
   #region <Setters>
	...
	/// @argument			position {int}
	/// @argument			count? {int}
	/// @description		Delete a part of the string, starting from the character at the specified
	///						position and continuing to the right for the specified number of
	///						characters or its end.
	...
	/// @description		Add to the string or replace it with one containing a number formatted
	///						with a specified amount of displayed places for whole and decimal digits.
	///						If there is less whole numbers than specified, the remaining places will
	///						be occupied by spaces added to the left. If there is less decimal numbers
	///						than specified, the remaining places will be occupied with zeros to the
	///						right.
	static formatNumber = function(_number, _wholeNumberPlaces, _decimalPlaces, _replace = false)
	{
		try
		{
	...
	/// @argument			replace? {bool}
	/// @description		Add to this string or replace it with a readable representation of the
	///						specified struct, listing its property names and their values.
	...
	/// @argument			substring {any:string}
	/// @argument			position {int}
	/// @description		Insert a specified substring to the left of the specified character.
	...
	/// @argument			number {int}
	/// @argument			separator? {any:string}
	/// @description		Add the specified number of copies of the string to itself, either as it
	///						is or with added separator before the copy.
	...
	/// @argument			count? {int}
	/// @description		Replace the specified target parts of the string with specified substitute
	///						string, either in its all instances or the specified amount of times, by
	///						searching for them from the start of the string to its end.
	...
	/// @description		Remove the specified characters from the start and the end of the string
	///						until a different character is detected. If no characters are specified,
	///						whitespace will be removed instead. Either side of the string can be
	///						specified to not be affected.
	static trim = function(_trimmedChar, _keep_left = false, _keep_right = false)
	{
		try
		{
			...
			var __charCheck = ((_trimmedChar != undefined) ? self.charEquals : self.charIsWhitespace);
	...
	/// @argument			position {int}
	/// @argument			byte {int}
	/// @description		Replace the character at the specified position by other one, specified
	///						with UTF-8 byte value.
	...
	/// @description		Set all letters from the English alphabet in the string to their
	///						lowercase variation.
	...
	/// @description		Set the first character of the string to its uppercase variation if it is
	///						a letter from the English alphabet.
	static capitalize = function()
	{
		var _string = string(ID);
		ID = (string_upper(string_char_at(_string, 1)) + string_delete(_string, 1, 1));
   ...
   #region <Execution>
	...
	/// @description		Pause the execution of the application to display the string in the
	///						message box handled by the build target if it supports it.
   ...
   #region <Conversion>
	...
	///						Content will be represented with the string of this constructor, where
	///						linebreaks are replaced with spaces if multiline usage is not specified.
	static toString = function(_multiline = false, _elementLength = 30, _mark_cut  = "...")
	{
		...
		if ((_elementLength != all) and (string_length(_string) > _string_lengthLimit_cut))
		{
	...
	/// @description		Set the string to one created from connecting all values of the specified
	///						array. A specified connector string can be included in between parts of
	///						the string.
	...
	/// @description		Save the string by creating or fully overwriting a text file at the
	///						specified path. 
	...
	static fromFile = function(_path)
	{
		...
		catch (_exception)
		{
			ErrorReport.report([other, self, "fromFile()"],
							   ("Attempted to parse a nonexistent or inacessible file at path: " +
								"{" + string(_path) + "}"));
		}
		finally
		{
	...
	/// @description		Set the string to the contents of a file with JSON formatting at the
	///						specified path and return contents of that file as a struct.
			...
			if ((is_handle(_buffer)) and (buffer_exists(_buffer)))
			{
				...
				catch (_exception)
				{
					ErrorReport.report([other, self, "fromJSON()"],
									   ("Attempted to parse a file with invalid JSON formatting at " +
										"path: " +
										"{" + string(_path) + "}"));
				}
			}
