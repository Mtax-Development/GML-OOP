...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
			...
			else
			{
				ErrorReport.report([other, self, "create()"],
								   ("Attempted to create a Surface of invalid size: " + "\n" +
									"Self: " + "{" + string(self) + "}" + "\n" +
									"Size: " + "{" + string(size) + "}" + "\n" +
									"Replacing the size property with a single pixel size and " +
									"creating the Surface with it."));
	...
	/// @description		Copy content of other Surface to a specified location on this Surface.
				...
				if (_other_part != undefined)
				{
					if (!self.isFunctional())
					{
						ErrorReport.report([other, self, "copy()"],
										   ("Attempted to copy part of a Surface to an invalid " +
											"Surface: " + "\n" +
											"Self: " + "{" + string(self) + "}" + "\n" +
											"Other: " + "{" + string(_other) + "}" + "\n" +
					...
					surface_copy_part(ID, _location.x, _location.y, _other_value, _other_part.x1,
									  _other_part.y1, _other_part.x2, _other_part.y2);
				}
				else
				{
					self.create();
   ...
   #region <Execution>
	...
	/// @description		Draw this Surface or a part of it to the currently active or specified
	///						Surface.
	static render = function(_location, _scale, _angle, _color = c_white, _alpha = 1, _part, _origin,
							 _target)
	{
		...
		try
		{
			if (self.isFunctional())
			{
				...
				draw_surface_general(ID, _part_x1, _part_y1, _part_x2, _part_y2, _location_x, 
									 _location_y, _scale_x, _scale_y, _angle_value, _color_topLeft,
									 _color_topRight, _color_bottomRight, _color_bottomLeft, _alpha);
	...
	/// @argument			color? {int:color}
	/// @argument			alpha? {real}
	/// @description		Draw this Surface tiled across the target created Surface or the entire
	///						Room if none is set.
			...
			case true:
				if (!self.isFunctional())
				{
					ErrorReport.report([other, self, "setActive()"],
									   ("Attempted to set an invalid Surface as the render target: " +
										"{" + string(ID) + "}" + "\n" +
   ...
   #region <Conversion>
	...
	/// @argument			part? {Vector4}
	/// @description		Save the content of this Surface or a rectangular part of it to the
	///						specified PNG file.
	...
	/// @argument			offset? {int}
	/// @description		Copy information from the specified Buffer to this Surface, starting from
	///						either its beginning or the specified byte offset.
