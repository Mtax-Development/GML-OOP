...
///						of its corner curves, made of series of triangles, the amount of which is
///						increased by the specified precision.
//						
//						Construction types:
//						- New constructor
		...
		if (argument_count > 0)
		{
			if (is_instanceof(argument[0], RoundRectangle))
			{
				...
				location = ((is_instanceof(_other.location, Vector4)) ? new Vector4(_other.location)
																	  : _other.location);
				radius = ((is_instanceof(_other.radius, Vector2)) ? new Vector2(_other.radius)
																  : _other.radius);
			...
			else
			{
				...
				outline_alpha = (((argument_count > 6) and (argument[6] != undefined)) ? argument[6]
																					   : 1);
				precision = (((argument_count > 7) and (argument[7] != undefined)) ? argument[7]
																				   : 24);
			}
		}
