	...
	static construct = function()
	{
		...
		if (argument_count > 0)
		{
			...
			else
			{
				...
				location = argument[0];
				radius = (((argument_count > 1) and (argument[1] != undefined)) ? argument[1] : 0);
   ...
   #region <Getters>
		...
		try
		{
			...
			if (_list)
			{
				...
				if ((is_real(_excludedInstance)) and (instance_exists(_excludedInstance)))
				{
					...
					{
						collision_circle_list(_location.x, _location.y, _radius, _object, _precise,
											  true, _list.ID, _listOrdered);
					}
				}
				else
				{
					collision_circle_list(_location.x, _location.y, _radius, _object, _precise, false,
			...
			else
			{
				if ((is_real(_excludedInstance)) and (instance_exists(_excludedInstance)))
				{
					with (_excludedInstance)
					{
						return collision_circle(_location.x, _location.y, _radius, _object, _precise,
												true);
					}
				}
				else
										...
										_precision = precision)
	{
		...
		try
		{
			...
			var _center = [_location.x, _location.y];
			var _i = 0;
			repeat (_precision)
			{
				var _trio = (_i * 3);
				...
				_result[(_trio + 1)] = [(_location.x + (_radius * _curve_cos[_i])),
										(_location.y + (_radius * _curve_sin[_i]))];
				_result[(_trio + 2)] = [(_location.x + (_radius * _curve_cos[(_i + 1)])),
										(_location.y + (_radius * _curve_sin[(_i + 1)]))];
   ...
   #region <Conversion>
									 ...
									 _outline = all, _vertexBuffer)
	{
		...
		try
		{
			...
			var _primitive = self.getPrimitiveRenderData(_location, _radius, _fill_color, _fill_alpha,
