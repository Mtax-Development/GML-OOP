...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	/// @description		Set the value operated by this parser to the specified one or one of the
	///						specified parser, by ensuring it is an array. An already existing array
	///						will be set as a reference, not as a copy, so changes in it  will be
	///						reflected in any variable or parser that have it assigned.
	...
	/// @argument			size? {int}
	/// @argument			value? {any}
	/// @description		Replace the array with a newly created array of the specified size filled
	...
	/// @description		Copy to the array the specfied number of elements or all of them from the
	///						specified position in the source array to the specified target position or
	///						the beginning of either array if unspecified.
	///						If the specified target position is negative, the values will be copied to
	///						that absolute position of the target backwards. If the specified number of
	///						values to copy from the source is negative, these values will be copied
	///						backwards from the end of source. Values at already occupied positions
	///						will be overwritten. Positions exceeding the target array size will be set
	///						to {undefined} if values were not copied into them.
	///						Condition functions can be specified and provided each candidate value as
						   ...
						   __condition_copy, __condition_execute)
	{
		try
		{
			...
			or (__condition_copy != undefined) or (__condition_execute != undefined))
			{
				...
				repeat (min((_count_abs ?? _remaining_position_count), _remaining_position_count))
				{
					try
					{
	...
	/// @description		Append to the array the specified values or all values of the specified
	///						structures.
				...
				else if (is_struct(_data))
				{
					switch (instanceof(_data))
					{
						case "ArrayParser":
							...
							{
								array_copy(ID, array_length(ID), _data.ID, 0, array_length(_data.ID));
							}
							else
							{
   ...
   #region <Getters>
	...
	/// @returns			{bool}
	/// @description		Check if the array contains a value fulfilling the specified condition
	///						function by causing it to return true at least once or for all cases as it
	///						is executed once for each value of the array.
	...
	static equals = function(_other)
	{
		try
		{
			return array_equals(ID, ((is_instanceof(_other, ArrayParser)) ? _other.ID : _other));
		}
		catch (_exception)
		{
	...
	/// @returns			{any[]}
	/// @description		Return all values among the array and specified values and arrays in a new
	///						array that does not contain duplicate values.
	static getUniqueValues = function()
	{
		...
		try
		{
			...
			repeat (argument_count)
			{
				...
				var _value = ((is_instanceof(_argument, ArrayParser)) ? _argument.ID : _argument);
	...
	/// @returns			{any[]}
	/// @description		Return all values among the array and specified values and arrays in a new
	///						array that does not contain duplicate values and only values present in
	///						all arguments.
	...
	/// @description		Return the first value in the array or {undefined} if the array does not
	///						exists or is empty.
	...
	/// @returns			{any|undefined}
	/// @description		Return the last value in the array or {undefined} if the array does not
	///						exists or is empty.
	...
	/// @argument			value {any}
	/// @returns			{int}
	/// @description		Return the first found position of the specified value or -1 if the value
	///						does not exist.
	...
	/// @argument			value {any}
	/// @returns			{int}
	/// @description		Return the last found position of the specified value or -1 if the value
	///						does not exist.
	...
	/// @argument			argument? {any}
	/// @returns			{int[]}
	/// @description		Return an array with all positions of values fulfilling the specified
	///						condition function by causing it to return true.
	///						The following arguments will be provided to the function and can be
	///						accessed in it by using their name or the argument array:
	///						- argument[0]: _i {int}
	///						- argument[1]: _value {any}
	...
	/// @argument			argument? {any}
	/// @returns			{any} | On error: {undefined}
	/// @description		Execute a function to compare each value in the array with the previous
	///						return value and return the final of these values. The comparison starts
	///						from either the specified initial value or the first value of the array.
	///						The following arguments will be provided to the function and can be
	///						accessed in it by using their name or the argument array:
   ...
   #region <Setters>
	...
	/// @description		Set the number of elements in the array to the specified one. If the
	///						specified size is lower than current, values from the end will be
	///						removed. If the specified size is higher than current, values in new
	///						positions will be set to the specified default value or {undefined} if
	///						unspecified.
	static setSize = function(_size, _value_default)
	{
		try
		{
			...
			if (_size > _size_current)
			{
				...
				array_copy(ID, _size_current, array_create(_count, _value_default), 0, _count);
			}
			else
			{
   ...
   #region <Execution>
	...
	/// @argument			value {any}
	/// @argument			position {int}
	/// @description		Set a specified position of the array to specified value and any empty
	...
	/// @argument			position {int}
	/// @argument			value... {any}
	/// @description		Add one or more values to the specified positions of the array and push
	///						values on that position and after it forward by the number of added
	///						values. Empty positions before the specified one will be set to 0.
	...
	/// @description		Remove any number of values from the arraystarting from the specified
	///						position and return them. If multiple values were successfully removed,
	///						they will be returned in an array. If no values were removed, {undefined}
	///						will be returned.
	...
	/// @returns			{int}
	/// @description		Remove all occurences of the specified value and return the number of
	///						removed values. If an ArrayParser is specified as the value, all
	///						occurences of its shared values will be removed.
	...
	/// @description		Sort the values in the array in the specified order, in a way depending on
	///						the type of sorting specified:
	///						- {bool}: True for ascending and false for descending. All array values
	///								  should be either numbers or strings.
	///						- {function}: The specified function must accept two arguments to compare
	///									  every value in the array with each other in pairs, then
	///									  return the result as a number with following meanings:
