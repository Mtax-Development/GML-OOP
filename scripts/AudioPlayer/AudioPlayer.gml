...
//  @feather	ignore all
{
	#region [Methods]
		#region <Management>
			...
			{
				return (((is_instanceof(audio, Audio)) and (audio.isFunctional())) and
						(is_bool(loop)) and (is_real(pitch)) and (is_real(gain)) and
						(is_real(offset)) and (is_real(priority)) and (is_bool(singular)));
			}
			...
			static toString = function(_multiline = false, _full = false)
			{
				...
				if (_full)
				{
					...
					if ((is_instanceof(audio, Audio)) and (audio.isFunctional()))
					{
						...
						_string_priority = ((is_real(priority)) ? string(priority)
																: string(audio.priority));
					}
					else
					{
