...
/// @argument			value? {real}
/// @description		Constructs a Container for a value enclosed in the specified Range.
//						
//						Construction types:
//						- New constructor
...
//  @feather	ignore all
{
  #region [Methods]
	...
	static construct = function()
	{
		...
		if ((argument_count > 0) and (argument[0] != undefined))
		{
			...
			else
			{
				...
				range = argument[0];
				var _value = (((argument_count > 1) and (argument[1] != undefined)) ? argument[1]
																					: range.minimum);
   ...
   #region <Getters>
	...
	/// @returns			{bool}
	/// @description		Check if the value is the same as the specified number or value of a
	///						specified Ranged Value. Those values can be specified in an array to check
	///						if any of them matches all of its values with the ones of this
	///						constructor.
	...
	/// @description		Return the percentage value representing the value inside of the Range as
	///						a numerical value, in which one whole number is one full percentage.
   ...
   #region <Setters>
	...
	/// @description		Modify the value by the specified number, then wrap it to the furthest
	///						boundary if it is outside of the Range. The value will also be wrapped if
	///						inclusive wrapping is specified and the value equals the Range.
	static modifyWrap = function(_value, _inclusive = false)
	{
		try
		{
			...
			_result = (((((_result - range.minimum) mod _rangeDifference) + _rangeDifference) mod
						_rangeDifference) + range.minimum);
	...
	/// @argument			value {real}
	/// @description		Modify the value by the specified number, then continuously bounce it back
	///						towards the Range if it would exceed its boundary.
	...
	/// @argument			value {real}
	/// @returns			{real}
	/// @description		Set the value to a position within the Range of the specified precentage.
	static interpolate = function(_value)
	{
		try
		{
			value = clamp(lerp(range.minimum, range.maximum, _value), range.minimum, range.maximum);
		}
		catch (_exception)
		{
