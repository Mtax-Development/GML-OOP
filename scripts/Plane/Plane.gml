...
///						Shape. The specified location is its center, from which it is scaled.
...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
			...
			if (is_instanceof(argument[0], Plane))
			{
				...
				sprite = ((is_instanceof(_other.sprite, Sprite))
						  ...
						  : ((is_instanceof(_other.sprite, SpriteRenderer))
							 ? new SpriteRenderer(_other.sprite) : _other.sprite));
				color = _other.color;
				alpha = _other.alpha;
	...
	static isFunctional = function()
	{
		return (((is_instanceof(location, Vector3)) and (location.isFunctional())) and
				...
				((is_real(color)) or ((is_instanceof(color, Color4)) and (color.isFunctional()))) and
				(is_real(alpha)));
	}
   ...
   #region <Getters>
										...
										_sprite = sprite)
	{
		...
		try
		{
			...
			if (is_instanceof(_sprite, SpriteRenderer))
			{
				...
				{
					_scale = new Scale((_sprite.scale.x * _scale.x), (_sprite.scale.y * _scale.y));
				}
			...
			if (is_instanceof(_sprite, Sprite))
			{
				...
				_uv_order = [[_uv[0], _uv[1]], [_uv[0], _uv[3]], [_uv[2], _uv[1]], [_uv[2], _uv[3]]];
	...
	/// @argument			angle? {EulerAngle}
	/// @returns			{real[]:matrix}
	/// @description		Return current transformation matrix of this Shape, using its data or its
	...
	/// @returns			{int[+]}
	/// @description		Return an array containing multipliers for direction of offsets used in
	///						calculating position of each vertex of this Shape.
	...
	///						using its current data or temporarily replaced parts. Data will be
	///						represented at following array positions:
	///						- array[0]: primitive type {constant:pr_*}
	///						- array[1]: vertex locations {real[+]}
	///						- array[2]: texture data {any[+]}
	static getPrimitiveRenderData = function(_location = location, _scale = scale, _angle = angle,
											 _sprite = sprite)
	{
		try
		{
							 ...
							 _color = color, _alpha = alpha)
	{
		...
		try
		{
			if (self.isFunctional())
			{
				...
				matrix_set(matrix_world, matrix_multiply(_matrix_original, self.getTransform()));
				...
				if (is_instanceof(sprite, Sprite))
				{
					...
					var _sprite_scale = new Scale(((scale.x * 2) / sprite_get_width(sprite.ID)),
												  ((scale.y * 2) / sprite_get_height(sprite.ID)));
					var _origin = new Vector2((_sprite_size_x * 0.5), (_sprite_size_y * 0.5));
					
					sprite.render(new Vector2(0, 0), 0, _sprite_scale, undefined, color, alpha,
								  undefined, _origin);
				}
				else if (is_instanceof(sprite, SpriteRenderer))
				{
					...
					var _sprite_scale = new Scale((sprite.scale.x * ((scale.x * 2) / _sprite_size_x)),
												  (sprite.scale.y * ((scale.y * 2) / _sprite_size_y)));
					var _origin = new Vector2((_sprite_size_x * 0.5), (_sprite_size_y * 0.5));
					
					sprite.render(undefined, _offset, undefined, _sprite_scale, undefined, undefined,
								  undefined, undefined, _origin);
				}
				else
				{
					var _vertex_sign = self.getVertexSign();
					...
					draw_primitive_begin(pr_trianglestrip);
					{
						...
						repeat (array_length(_vertex_sign))
						{
							...
							draw_vertex_color((scale.x * _vertex_sign_current[0]),
											  (scale.y * _vertex_sign_current[1]), color, alpha);
