...
///						first-in-first-out model, which disallows order manipulation.
//						
//						Construction types:
//						- New constructor
...
//  @feather	ignore all
{
  #region [Methods]
   ...
   #region <Getters>
	...
	/// @description		Return the head value of this Queue, which is the one that would be
	///						removed first.
	...
	/// @returns			{any|undefined}
	/// @description		Return the tail value of this Queue, which is the one that would be
	///						removed last.
	///						Returns {undefined} if this Queue does not exists or is empty.
   ...
   #region <Execution>
	...
	///						its copy, without modifying the original to read the values.
	...
	/// @description		Remove any number of values from this Queue and return them. If more than
	///						one value were removed, they will be returned in an array.
	///						Returns {undefined} if this Queue does not exists or is empty.
   ...
   #region <Conversion>
	...
	/// @description		Add values from the specified array to this Queue, starting from either
	...
	static fromArray = function(_array, _startFromEnd = false)
	{
		...
		else
		{
			ErrorReport.report([other, self, "fromArray()"],
							   ("Attempted to convert an invalid array to a Data Structure: " +
								"{" + string(_array) + "}"));
		}
	...
	///						Mark it as "legacy" if that string was created in the old version of
