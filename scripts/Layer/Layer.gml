...
/// @description		Constructs a Layer Resource, used to group graphical elements and sort their
///						rendering depth.
//						
//						Construction types:
//						- New constructor
...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	static construct = function()
	{
		...
		if ((argument_count > 0) and (argument[0] != undefined))
		{
			if (is_instanceof(argument[0], Layer))
			{
				...
				var _elementType = [SpriteElement, BackgroundElement, TilemapElement, ParticleSystem];
				var _i = [0, 0];
	...
	///						If there are persistent instances bound to this Layer, they will not be
	///						destroyed, unless the destruction is forced, but all Data Structures of
	///						this Layer will be destroyed in either case.
   ...
   #region <Setters>
	...
	/// @description		Change the rendering offset for Elements of this Layer, except of object
	///						instances.
	...
	/// @description		Set the horizontal and vertical speed of Elements of this Layer, except of
	///						object instances.
	...
	///						Instances on a Layer that is not visible will not have their Draw Events
	///						executed.
	...
	/// @argument			depth {int}
	/// @description		Set the depth of this Layer, which dictates its render sorting.
	...
	/// @description		Set a Shader to be applied to every Element of this Layer.
	...
	/// @description		Set a function to be called before this Layer begins rendering.
	...
	/// @argument			function {function}
	/// @description		Set a function to be called after this Layer finishes rendering.
   ...
   #region <Execution>
	...
	/// @returns			{undefined}
	/// @description		Destroy the specified or all instances or objects of this Layer. If {all}
	///						is specified, the instances will be destroyed instantly. Otherwise, they
	///						will remain until the next application frame.
	...
	/// @description		Activate or deactivate all instances on this Layer.
   ...
   #region <Conversion>
	...
	///						Content will be represented with the name and depth of this Layer.
		...
		if (self.isFunctional())
		{
			...
			else
			{
				...
				_string = ("Name: " + string(name) + _mark_separator +
						   ...
						   "Instances Paused: " + string(instancesPaused) + _mark_separator +
						   "Function (Draw Begin): " + _string_function_drawBegin + _mark_separator +
						   "Function (Draw End): " + _string_function_drawEnd + _mark_separator +
	...
	function SpriteElement() constructor
	{
	  #region [[Methods]]
	   #region <<Management>>
			...
			if ((argument_count > 0) and (argument[0] != undefined))
			{
				...
				switch (_instanceof_other)
				{
					case _instanceof_self:
						...
						location = new Vector2(_other.location);
						ID = layer_sprite_create(parent.ID, location.x, location.y, sprite.ID);
					...
					default:
						...
						location = new Vector2(layer_sprite_get_x(ID), layer_sprite_get_y(ID));
						angle = new Angle(layer_sprite_get_angle(ID));
						scale = new Scale(layer_sprite_get_xscale(ID), layer_sprite_get_yscale(ID));
		...
		static isFunctional = function()
		{
			return ((is_real(ID)) and (is_instanceof(parent, Layer)) and (parent.isFunctional()) and
		...
		static changeParent = function(_other)
		{
			if (self.isFunctional())
			{
				...
				else
				{
					ErrorReport.report([other, parent, "SpriteElement", "changeParent()"],
									   ("Attempted a parent change to an invalid Layer:" + "\n" +
										"Self: " + "{" + string(self) + "}" + "\n" +
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "changeParent()"],
								   ("Attempted a parent change on invalid Element or Parent:" + "\n" +
									"Self: " + "{" + string(self) + "}" + "\n" +
									"Parent: " + "{" + string(parent) + "}"));
			}
		...
		{
			if ((is_instanceof(parent, Layer)) and (is_instanceof(parent.spriteList, List)))
			{
				parent.spriteList.removeValue(self);
			}
	   ...
	   #region <<Setters>>
		...
		static setSprite = function(_sprite)
		{
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "setSprite()"], _exception);
			}
		...
		static setScale = function(_scale)
		{
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "setScale()"], _exception);
			}
		...
		static setColor = function(_color)
		{
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "setColor()"], _exception);
			}
		...
		static setAlpha = function(_alpha)
		{
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "setAlpha()"], _exception);
			}
		...
		static setFrame = function(_frame)
		{
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "setFrame()"], _exception);
			}
		...
		static setSpeed = function(_speed)
		{
			...
			{
				ErrorReport.report([other, parent, "SpriteElement", "setSpeed()"], _exception);
			}
	...
	function BackgroundElement() constructor
	{
	  #region [[Methods]]
	   #region <<Management>>
			...
			if ((argument_count > 0) and (argument[0] != undefined))
			{
				...
				switch (_instanceof_other)
				{
					case _instanceof_self:
						...
						scale = ((is_instanceof(_other.sprite, Scale)) ? new Scale(_other.scale)
																	   : _other.scale);
						color = _other.color;
						alpha = _other.alpha;
						frame = _other.frame;
		...
		{
			return ((is_real(ID)) and (is_instanceof(parent, Layer)) and (parent.isFunctional()) and
					(layer_background_exists(parent.ID, ID)));
		}
		...
		static changeParent = function(_other)
		{
			if (self.isFunctional())
			{
				...
				else if (((is_real(_other)) or (is_string(_other))) and (layer_exists(_other)))
				{
					parent.backgroundList.removeValue(self);
					parent = undefined;
		...
		static destroy = function()
		{
			if ((is_instanceof(parent, Layer)) and (is_instanceof(parent.backgroundList, List)))
			{
				parent.backgroundList.removeValue(self);
			}
	   ...
	   #region <<Setters>>
		...
		static setSprite = function(_sprite)
		{
			...
			{
				ErrorReport.report([other, parent, "BackgroundElement", "setSprite()"], _exception);
			}
		...
		static setScale = function(_scale)
		{
			...
			{
				ErrorReport.report([other, parent, "BackgroundElement", "setScale()"], _exception);
			}
		...
		/// @description		Set the tiling properties of this Background Element for horizontal
		///						and vertical tiling respectively.
		static setTiled = function(_tiled_x, _tiled_y)
		{
			...
			{
				ErrorReport.report([other, parent, "BackgroundElement", "setTiled()"], _exception);
			}
	...
	function TilemapElement() constructor
	{
	  #region [[Methods]]
	   #region <<Management>>
		...
		static construct = function()
		{
			...
			if ((argument_count > 0) and (argument[0] != undefined))
			{
				...
				if (_instanceof_other == _instanceof_self)
				{
					...
					size = _other.size;
					ID = layer_tilemap_create(parent.ID, location.x, location.y, tileset, size.x,
											  size.y);
				}
		...
		static isFunctional = function()
		{
			return ((is_handle(ID)) and (is_instanceof(parent, Layer)) and (parent.isFunctional()) and
					(layer_tilemap_exists(parent.ID, ID)));
		}
		...
		static destroy = function()
		{
			if ((is_instanceof(parent, Layer)) and (is_instanceof(parent.tilemapList, List)))
			{
		...
		/// @description		Set all Tiles in this Tilemap to empty or the specified Tile Data.
		static clear = function(_tileData = 0)
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "clear()"], _exception);
			}
		...
		static changeParent = function(_other)
		{
			if (self.isFunctional())
			{
				...
				{
					ErrorReport.report([other, parent, "TilemapElement", "changeParent()"],
									   ("Attempted a parent change to an invalid Layer:" + "\n" +
										"Self: " + "{" + string(self) + "}" + "\n" +
										"Parent: " + "{" + string(parent) + "}" + "\n" +
										"Other: " + "{" + string(_other) + "}"));
	   ...
	   #region <<Getters>>
		...
		static getFrame = function()
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "getFrame()"], _exception);
			}
		}
		
		/// @returns			{int} | On error: {int:-1}
		/// @description		Return the bit mask value for this Tilemap or 0 if there is none.
		static getMask = function()
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "getMask()"], _exception);
			}
		...
		/// @description		Return a TileData referring to the Tile in a cell at the specified
		///						location.
		static getTileInCell = function(_location)
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "getTileInCell()"], _exception);
			}
		...
		/// @argument			location {Vector2}
		/// @returns			{Layer.TilemapElement.TileData}
		/// @description		Return a TileData referring to the Tile at specified point in the
		///						Room.
	   ...
	   #region <<Setters>>
		...
		static setMask = function(_mask)
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "setMask()"], _exception);
			}
		...
		static setTileset = function(_tileset)
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "setTileset()"], _exception);
			}
		...
		/// @argument			size {Vector2}
		/// @description		Set the size of this Tilemap, which is the number of Tile cells it
		///						will use.
		static setSize = function(_size)
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "setSize()"], _exception);
			}
	   ...
	   #region <<Execution>>
		...
		/// @argument			location? {Vector2}
		/// @description		Execute the draw of this Tilemap, independent of its draw executed by
		///						its Layer.
		static render = function(_location = new Vector2(0, 0))
		{
			...
			{
				ErrorReport.report([other, parent, "TilemapElement", "render()"], _exception);
			}
		...
		/// @argument			tileData? {Layer.TilemapElement.TileData|int:tileData}
		/// @returns			{bool}
		/// @description		Set the Tile at the specified cell to a Tile referred to by the
		///						specified Tile Data and return a boolean confirming whether the
		///						operation was a success or not. 
		///						If Tile Data is not provided or specified as 0, the cell will be
		///						cleared.
		static setTileInCell = function(_location, _tileData = 0)
		{
			try
			{
				...
				return ((_tileData != (-1)) ? tilemap_set(ID, _tileData, _location.x, _location.y)
											: false);
			}
			catch (_exception)
			{
				ErrorReport.report([other, parent, "TilemapElement", "setTileInCell()"], _exception);
			}
		...
		/// @argument			location {Vector2}
		/// @argument			tileData? {Layer.TilemapElement.TileData|int:tileData}
		/// @description		Set the Tile at the specified point in space to a Tile referred to by
		///						the specified Tile Data and return a boolean for confirmation whether
		...
		static setTileAtPoint = function(_location, _tileData = 0)
		{
			try
			{
				...
				return ((_tileData != (-1))
						? tilemap_set_at_pixel(ID, _tileData, _location.x, _location.y) : false);
			}
			catch (_exception)
			{
				ErrorReport.report([other, parent, "TilemapElement", "setTileAtPoint()"], _exception);
			}
	   ...
	   #region <<Conversion>>
		...
		/// @description		Create a string representing this constructor.
		///						Overrides the string() conversion.
		///						Content will be represented with the properties of this Tilemap
		///						Element.
		static toString = function(_multiline = false)
		{
			...
			if (self.isFunctional())
			{
				...
				var _string = ("ID: " + string(ID) + _mark_separator +
							   "Tileset: " + tileset_get_name(tileset) + _mark_separator +
							   "Location: " + string(location) + _mark_separator +
							   "Size: " + string(size));
		  ...
		  #region [[[Methods]]]
		   #region <<<Management>>>
			...
			static clear = function()
			{
				if (!self.isFunctional())
				{
					ErrorReport.report([other, "Layer", "TilemapElement", "TileData", "clear()"],
									   ("Attempted to set a property of an invalid Tile:" + "\n" +
										"Self: " + "{" + string(self) + "}" + "\n" +
										"Parent: " + "{" + string(parent) + "}" + "\n" +
		   ...
		   #region <<<Getters>>>
			...
			static isEmpty = function()
			{
				if (!self.isFunctional())
				{
					ErrorReport.report([other, "Layer", "TilemapElement", "TileData", "isEmpty()"],
									   ("Attempted to set a property of an invalid Tile:" + "\n" +
										"Self: " + "{" + string(self) + "}" + "\n" +
										"Parent: " + "{" + string(parent) + "}" + "\n" +
										"Clearing the Tile ID to 0."));
			...
			/// @returns			{bool}
			/// @description		Check if this Tile Data refers to a Tile that was mirrored
			///						horizontally.
			...
			/// @returns			{bool} | On error: {undefined}
			/// @description		Check if this Tile Data refers to a Tile that was rotated by 90
			///						degrees.
		   ...
		   #region <<<Setters>>>
			...
			/// @argument			index {int}
			/// @description		Change the Tile this Tile Data refers to, based on the index of
			///						its tileset.
			...
			/// @description		Set the horizontal mirroring property of the Tile this Tile Data
			///						refers to.
			static setMirrorX = function(_mirror)
			{
				...
				{
					ErrorReport.report([other, "Layer", "TilemapElement", "TileData", "setMirrorX()"],
										_exception);
				}
			...
			/// @argument			mirror {bool}
			/// @description		Set the vertical mirroring property of the Tile this Tile Data
			///						refers to.
			static setMirrorY = function(_mirror)
			{
				...
				{
					ErrorReport.report([other, "Layer", "TilemapElement", "TileData", "setMirrorY()"],
									   _exception);
				}
			...
			/// @description		Set the 90 degree rotation property of the Tile this Tile Data
			///						refers to.
			static setRotate = function(_rotate)
			{
				...
				{
					ErrorReport.report([other, "Layer", "TilemapElement", "TileData", "setRotate()"],
									   _exception);
				}
		   ...
		   #region <<<Execution>>>
			...
			/// @argument			location? {Vector2}
			/// @argument			frame? {int}
			/// @description		Execute the draw of the Tile this Tile Data refers to, independent
			///						of its draw handled by its Layer.
			static render = function(_location = new Vector2(0, 0), _frame = 0)
			{
				try
				{
					if (!self.isFunctional())
					{
						ErrorReport.report([other, "Layer", "TilemapElement", "TileData", "render()"],
										   ("Attempted to set a property of an invalid Tile:" + "\n" +
											"Self: " + "{" + string(self) + "}" + "\n" +
											"Parent: " + "{" + string(parent) + "}" + "\n" +
											"Clearing the Tile ID to 0."));
		   ...
		   #region <<<Conversion>>>
			...
			/// @description		Create a string representing this constructor.
			///						Overrides the string() conversion.
			///						Content will be represented with the ID of this Tile Data.
			static toString = function(_multiline = false)
			{
	  ...
	  #region [[Methods]]
	   #region <<Management>>
		...
		static construct = function()
		{
			...
			{
				var _instanceof_self = instanceof(self);
				var _instanceof_other = ((argument_count > 0) ? instanceof(argument[0]) : undefined);
				...
				{
					//|Construction type: New constructor.
					persistent = (((argument_count > 0) and (argument[0] != undefined)) ? argument[0]
																						: false);
					location = new Vector2(0, 0);
		...
		static clear = function()
		{
			...
			{
				ErrorReport.report([other, parent, "ParticleSystem", "clear()"], _exception);
			}
	   ...
	   #region <<Setters>>
		...
		static setLocation = function(_location)
		{
			...
			{
				ErrorReport.report([other, parent, "ParticleSystem", "setLocation()"], _exception);
			}
		...
		static setDrawOrder = function(_newerOnTop)
		{
			...
			{
				ErrorReport.report([other, parent, "ParticleSystem", "setDrawOrder()"], _exception);
			}
		...
		/// @description		Set whether created Particles are executed without the update call.
		static setAutomaticUpdate = function(_automaticUpdate)
		{
			...
			{
				ErrorReport.report([other, parent, "ParticleSystem", "setAutomaticUpdate()"],
								   _exception);
			}
		...
		/// @description		Set whether created Particles are executed without the render call.
		static setAutomaticRender = function(_automaticRender)
		{
			...
			{
				ErrorReport.report([other, parent, "ParticleSystem", "setAutomaticRender()"],
								   _exception);
			}
	   ...
	   #region <<Execution>>
		...
		/// @argument			count? {int}
		/// @description		Advance all actions of created Particles by the specified number of
		///						steps.
		static update = function(_count = 1)
		{
			...
			{
				ErrorReport.report([other, parent, "ParticleSystem", "update()"], _exception);
			}
	   ...
	   #region <<Conversion>>
		...
		static toString = function(_multiline = false, _full = false)
		{
			...
			if (self.isFunctional())
			{
				var _string = "";
				var _mark_separator = ((_multiline) ? "\n" : ", ");
				var _string_emitterNumber = (((is_instanceof(emitterList, List)) and
											  (emitterList.isFunctional()))
											 ? string(emitterList.getSize()) : string(undefined));
				...
				if (_full)
				{
					...
					_string = ("ID: " + string(ID) + _mark_separator+
							   ...
							   "Location: " + string(location) + _mark_separator +
							   "Automatic Update: " + string(automaticUpdate) + _mark_separator +
							   "Automatic Render: " + string(automaticRender) + _mark_separator +
							   "Draw Order: " + _string_drawOrder_newerOnTop + _mark_separator +
		...
		function ParticleEmitter() constructor
		{
		  #region [[[Methods]]]
		   #region <<<Management>>>
				...
				if (argument_count > 0)
				{
					if (argument[0] != undefined)
					{
						...
						var _instanceof_other = ((argument_count > 0)
												 ? instanceof(argument[0]) : undefined);
			...
			static isFunctional = function()
			{
				return ((is_handle(ID)) and
						(string_copy(string(instanceof(parent)), 1, 14) == "ParticleSystem") and
						(parent.isFunctional()) and (part_emitter_exists(parent.ID, ID)))
			}
			...
			/// @description		Set all properties of this Particle Emitter to default, except for
			///						its used Particle Type.
			static clear = function()
			{
				if (self.isFunctional())
				{
				...
				else if ((string_copy(string(instanceof(parent)), 1, 14) == "ParticleSystem")
				...
				{
					ErrorReport.report([other, "Layer", "ParticleSystem", "ParticleEmitter",
										"clear()"],
									   ("Attempted to clear an Element of an invalid Particle " +
										"System:" + "\n" +
										"Self: " + "{" + string(self) + "}" + "\n" +
										"Parent: " + "{" + string(parent) + "}"));
				}
		   ...
		   #region <<<Setters>>>
			...
			/// @description		Set the region in which the particles will be created.
			static setRegion = function(_location, _shape, _distribution)
			{
				try
				{
					part_emitter_region(parent.ID, ID, _location.x1, _location.x2, _location.y1,
										_location.y2, _shape, _distribution);
			...
			static setStreamEnabled = function(_streamEnabled)
			{
				...
				else
				{
					ErrorReport.report([other, "Layer", "ParticleSystem",
									   ...
									   ("Attempted to set properties on an invalid Element or " +
										"Particle System:" + "\n" +
										"Self: " + "{" + string(self) + "}" + "\n" +
										"Parent: " + "{" + string(parent) + "}"));
				}
		   ...
		   #region <<<Execution>>>
			...
			/// @description		Continuously create Particles using the stream configuration.
			...
			static toString = function(_multiline = false, _full = false)
			{
				...
				if (self.isFunctional())
				{
					...
					else
					{
						...
						switch (shape)
						{
							case ps_shape_rectangle: _string_shape = "Rectangle"; break;
							case ps_shape_ellipse: _string_shape = "Ellipse"; break;
							case ps_shape_diamond: _string_shape = "Diamond"; break;
							case ps_shape_line: _string_shape = "Line"; break;
						...
						_string = ("ID: " + string(ID) + _mark_separator +
								   "Particle Type: " + string(particleType) + _mark_separator +
								   "Location: " + string(location) + _mark_separator +
								   "Shape: " + _string_shape + _mark_separator +
								   "Distribution: " + _string_distribution + _mark_separator +
								   "Stream Enabled: " + string(streamEnabled) + _mark_separator +
