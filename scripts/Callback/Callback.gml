...
/// @description		Constructs a Handler storing a reference to a function or an array of them and
///						their arguments to execute them at later point.
///						Single value specified as an argument will be provided to each execution.
///						If multiple functions or methods are being executed, arguments specified in an
///						array will be provided to calls at respective array positions as single
...
//  @feather	ignore all
{
  #region [Methods]
   #region <Management>
	...
	/// @description		Remove references to functions or methods from this constructor, so its
	///						executions will have no effect when called. Argument data will also be
   ...
   #region <Getters>
	...
	/// @description		Return an array containing arguments of this Callback after the specified
	///						values. It can be used as argument replacement during execution of a
	///						Callback, so that each function called will be provided specified
	///						arguments before its respective arguments present in the argument property
	...
	static prependArgument = function()
	{
		...
		if (is_array(self.argument))
		{
			...
			if (_callback_isArray)
			{
				...
				repeat (max(array_length(ID), _argument_original_count))
				{
					if (_argument_original_count > _i)
					{
						...
						array_copy(_argument_nested, 0, _argument_array, 0, argument_count);
						
						if (is_array(_argument_original_current))
						{
							array_copy(_argument_nested, argument_count, _argument_original_current,
									   0, array_length(_argument_original_current));
						}
						else
						{
			...
			else
			{
				...
				array_copy(_result, argument_count, self.argument, 0, _argument_original_count);
			}
		}
		else
		{
   ...
   #region <Setters>
	...
	///						Callback or array, the first value of which is a function or method and
	///						second value is their arguments. Each of these array positions can be
	///						occupied by a nested array to specify multiple values.
   ...
   #region <Execution>
	...
	/// @description		Execute all functions and methods of this constructor in their order by
	///						providing them their respective arguments, if any. This execution can be
	///						additionally controlled by specifying a condition to prevent the operation
	///						when that condition is not fulfilled.
	static execute = function(_condition = true, _argument = self.argument, _scope = scope)
	{
		if ((ID != undefined) and (_condition))
		{
			...
			var _argument_direct = (((!_callback_isArray) and (_argument_isArray)) or
									((_argument_isArray) and (array_length(_argument) == 0)));
			var _callback_array = ((_callback_isArray) ? ID : [ID]);
			var _callback_count = array_length(_callback_array);
			var _argument_array = ((_argument_isArray) ? _argument
													   : array_create(_callback_count, _argument));
			...
			repeat (_callback_count)
			{
				var _callback_index = ((is_method(_callback_array[_i]))
									   ? method_get_index(_callback_array[_i]) : _callback_array[_i]);
   ...
   #region <Conversion>
							   ...
							   _mark_arraySeparator = ", ")
	{
		if (self.isFunctional())
		{
			...
			var _argument_direct = (((!_callback_isArray) and (_argument_isArray)) or
									((_argument_isArray) and (array_length(self.argument) == 0)));
			var _callback_array = ((is_array(ID) ? ID : [ID]));
			var _size = array_length(_callback_array);
			var _argument_array = ((_argument_isArray) ? self.argument
													   : array_create(_size, self.argument));
				...
				try
				{
					if (is_method(_callback_current))
					{
						...
						if (is_struct(_callback_owner))
						{
							...
							{
								var _separatorPosition = string_last_pos("@", _callback_owner_name);
